...
# Get the present study's EdgeR results, and associate with their beebase IDs for matching the older studies
present_study_data <- read_csv("data/edgeR_results.csv") %>% 
  dplyr::filter(Time == 8) %>%              # USES THE 8-HOUR CASTE DIFFERENCE AND EXPRESSION LEVEL RESULTS
  left_join(tbl(db, "gene_names") %>% 
              dplyr::select(gene_symbol, gene_name, entrez_id, beebase2) %>% collect(n=Inf), 
            by = c("Gene symbol" = "gene_symbol")) %>% 
  dplyr::select(beebase2, logFC, AveExpr, `Gene symbol`, entrez_id, gene_name) %>%
  dplyr::rename(`Upregulation in queen-destined larvae` = logFC,
         gene = beebase2) %>%
  filter(!is.na(gene))
...
merged_data <- read.delim("data/apis_gene_comparisons/Amel_AllData_012709.txt", 
  ...
  dplyr::rename(beebase1 = ID) %>%                              
  left_join(gene_names, by = "beebase1") %>% 
  dplyr::filter(!is.na(beebase2) & beebase2 != "-") %>% 
  dplyr::rename(gene = beebase2) %>% 
  dplyr::select(gene, CAI)
...
merged_data <- full_join(present_study_data, 
                       merged_data, by = "gene") %>%
...
merged_data <- merged_data %>% mutate(CG_OE = -log2(CG_OE))       
...
merged_data <- merged_data %>% 
  left_join(read_csv("data/apis_gene_comparisons/wojciechowski_histone_data/H3K4me3.csv") %>% 
              mutate(H3K4me3_caste = caste_difference) %>% dplyr::select(gene, H3K4me3_caste), by = "gene") %>%
  left_join(read_csv("data/apis_gene_comparisons/wojciechowski_histone_data/H3K27ac.csv") %>% 
              mutate(H3K27ac_caste = caste_difference) %>% dplyr::select(gene, H3K27ac_caste), by = "gene") %>%
  left_join(read_csv("data/apis_gene_comparisons/wojciechowski_histone_data/H3K36me3.csv") %>% 
              mutate(H3K36me3_caste = caste_difference) %>% dplyr::select(gene, H3K36me3_caste), by = "gene") %>%
