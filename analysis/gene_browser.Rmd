...
The code below loads libraries and the data we need, i.e. the methylation calls for each site from BWASP, each gene's exon boundaries from the file `Amel.pcg.gff3`, and the list of genes containing at least once significantly differentially methylated site in their exons, UTRs, introns, or promoter region (i.e. all the genes mentioned in `gene_meth_results_table`). Each plot is created from two ggplots, one showing the gene model and one showing the methylated sites using the same x-axis. All methylated CpG sites identified by BWASP are shown.
...
grid.arrange(
  ...
  plot_gene("LOC412046", plot=F, in_markdown = F),
  plot_gene("LOC412162", plot=F, in_markdown = F),
  left = "% methylated sites", bottom = "Genomic position"
) %>% ggsave(filename = "figures/gene_browser_fig.pdf", height = 13, width = 9)
```
