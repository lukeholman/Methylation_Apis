...
Here, we construct a sample-by-site matrix where each cell contains the propotion of methylated cytosines, containing all `r length(to_keep)` sites that were measured with 20x or better coverage in at least 10 samples. The data were then reduced to 16 components by non-metric multidimensional scaling (NMDS), preserving `r round(readRDS("output/meth_nmds_rs.rds")*100)`% of the variance in the original data. 
...
meth_matrix <- readRDS("data/Methylation_data/meth_matrix.rds")
...
meth_sample_heatmap <- function(meth_matrix){

  foc <- readRDS("data/Methylation_data/input_data_for_meth_network_CLEANED.rds")
  ...
  foc %>%
    pheatmap(col = myColor, breaks = myBreaks,
             ...
             cutree_cols = 7, cutree_rows = 4, 
             show_colnames = FALSE, polar = T)
}

meth_sample_heatmap(meth_matrix)
```
...
if(!file.exists("output/meth_eigengenes.rds")){
  ...
  meth_connectivity <- tibble( # finish this.....
