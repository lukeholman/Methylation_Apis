...
The animation shows the distribution of % methylation for all CpG sites of the specified type. Because the majority of CpG sites have no methylation in this species, we have truncated the y-axis so that the right tail of the distribution is visible. To obtain the data underlying this figure, we summed up the number of methylated and unmethylated sequencing reads for each site, across the four replicate samples for each caste/time combination, then found the % methylated reads. The histograms display the frequency in each of 101 bins, for QDL (red) and WDL (blue).

One can view non-animated versions of the the same figures in the next section. 
...
if(!file.exists("figures/site_meth_hist/combined_animation.gif")){
  ...
  make_meth_density_plots <- function(by_site, site_type){
    ...
    site_meth_counts <- by_site %>%
      ...
      group_by(caste, time, site) %>% # combine the reads over the 4 replicate samples...
    ...
    static_plot <- site_meth_counts %>%
      ...
      theme(legend.position = "none", 
            ...
            strip.background = element_blank(),
            panel.grid.major.x = element_blank()) + 
      xlab("Site methylation level (% mCpGs)") +
  ...
  ####################################################
  # Now make histogram from main methylation figure in the paper:
  data_8h <- by_site %>%
      filter(time == 8) %>%
    select()
  
  get_fig1_data <- function(site_kind){
    by_site %>%
      filter(time == 8) %>%
      filter_by_site(site_kind) %>%
      mutate(coverage = numC + numT) %>%
      dplyr::select(site, numC, coverage, caste) %>%
      group_by(caste, site) %>% # combine the reads over the 4 replicate samples...
      summarise(prop_meth = sum(numC) / sum(coverage), .groups = "drop") %>% 
      dplyr::select(caste, prop_meth) %>%
      collect(n = Inf) %>%
      mutate(site_type = site_kind)
  }
  fig1_data <- get_fig1_data("translated") %>% 
    bind_rows(get_fig1_data("promoter")) %>%
    bind_rows(get_fig1_data("intron")) %>%
    bind_rows(get_fig1_data("5UTR")) %>%
    bind_rows(get_fig1_data("3UTR")) %>%
    bind_rows(get_fig1_data("intergenic")) %>% 
    mutate(caste = replace(caste, caste =="t", "Time 0"),
           caste = factor(caste, c("Time 0", "Queen", "Worker")),
           site_type = replace(site_type, site_type == "translated", "Protein-coding regions"),
           site_type = replace(site_type, site_type == "promoter", "Promoter regions"),
           site_type = replace(site_type, site_type == "intergenic", "Intergenic regions"),
           site_type = replace(site_type, site_type == "intron", "Introns"),
           site_type = replace(site_type, site_type == "5UTR", "5'-UTRs"),
           site_type = replace(site_type, site_type == "3UTR", "3'-UTRs"),
           site_type = factor(site_type, c("Protein-coding regions", "Promoter regions", "Introns", "5'-UTRs", "3'-UTRs", "Intergenic regions")))
  

  make_part_fig1 <- function(dat, ylims){
    ...
    dat %>%
      ...
      scale_fill_manual(values = c("#b19cd9", queen_colour, worker_colour)) +
      coord_cartesian(ylim = c(0, ylims)) +
      theme_bw() + 
      theme(legend.position = "none", 
            text = element_text(family = nice_font),
            strip.background = element_blank(),
  ...
  p1 <- fig1_data %>%
    filter(site_type %in% c("Protein-coding regions", "Promoter regions", "Introns")) %>%
    make_part_fig1(ylims = 2300) + theme(axis.text.x = element_blank(), axis.ticks.x = element_blank())
  
  p2 <- fig1_data %>%
    filter(site_type %in% c("5'-UTRs", "3'-UTRs", "Intergenic regions")) %>%
    make_part_fig1(ylims = 125) + xlab("% mCpGs")
  
  nice_histo <- arrangeGrob(
    rbind(ggplotGrob(p1), ggplotGrob(p2)),
    left = "Number of CpG sites")
  
  saveRDS(nice_histo, "figures/histo_fig1.rds")
}
```
...
![](https://raw.githubusercontent.com/lukeholman/Methylation_Apis/master/figures/site_meth_hist/translated_plot.png)

#### Promoter regions
![](https://raw.githubusercontent.com/lukeholman/Methylation_Apis/master/figures/site_meth_hist/promoter_plot.png)

#### Introns
![](https://raw.githubusercontent.com/lukeholman/Methylation_Apis/master/figures/site_meth_hist/intron_plot.png)

#### 5'-UTRs
![](https://raw.githubusercontent.com/lukeholman/Methylation_Apis/master/figures/site_meth_hist/5UTR_plot.png)

#### 3'-UTRs

![](https://raw.githubusercontent.com/lukeholman/Methylation_Apis/master/figures/site_meth_hist/3UTR_plot.png)

#### Intergenic regions

![](https://raw.githubusercontent.com/lukeholman/Methylation_Apis/master/figures/site_meth_hist/intergenic_plot.png)
...
The following uses the results of the GLMs generated below, which estimates the difference in mean % methylation for QDL and WDL at each time point, for each site (as well as the standard error associated with this difference in means). These can be thought of as effect sizes, which describe the effect of caste on methylation at each site. 

In this section, we analyse those effect sizes using a linear model, in order to estimate the average effect size for sites of each type. This was accomplished by running a linear model with the following formula:

$$y_i = \alpha + \beta_1x_{1i} + \beta_1x_{2i} + \beta_1x_{3i} + \beta_1x_{4i} + \beta_1x_{5i} + \epsilon_i$$
Here, $y_i$ is the effect size for site $i$, $\alpha$ is the global intercept, the five $\beta$ terms are regression coefficients to be estimated, the five $x_i$ terms are binary predictor variables for site $i$, and $\epsilon_i$ is the residual (assumed to follow a Normal distribution). The five binary predictor variables describe whether the focal site is (or is not) in a coding region, a promoter, an intron, a 5'-UTR, or a 3'-UTR respectively; note that any one site can overlap more than one of these genomic features. Promoter regions were defined as regions that are up to 2000bp upstream or 200bp downstream of a gene. After fitting the model, we then used the estimated regression coefficients to find the estimated average effect size for a site that maps uniquely to each one of the five genomic features. One model was run for each time point (i.e. one model for each of the figure panels below).


### Comparing queen- and worker-destined larvae

```{r site_type_plots, message=FALSE, warning=FALSE, fig.width=5.3, fig.showtext=TRUE}
# Note: this file is produced below, but the results fit here more logically
foc_sites <- read_tsv("output/frequentist_GLM_meth_diff.txt") %>% 
  pull(Site) %>% unique()

plotting_data <- read_tsv("output/frequentist_GLM_meth_diff.txt") %>%
  left_join(tbl(db, "site_annotations") %>% 
              filter(site %in% foc_sites) %>% 
              dplyr::select(-exon_rank) %>%
              collect(), 
            by = c("Site" = "site")) %>%
  ...
  dplyr::select(Site, time, Queen, Worker, Caste_difference, starts_with("in_"))

rm(foc_sites)

# args should be like "timeQ", 2, 6 (for 2h vs 6h comp in QDL)
make_lm_estimates <- function(comparison, timepoint1, timepoint2){  
  
  if(comparison == "between_castes"){
    facet_label <- paste(timepoint1, "h post-grafting", sep = "")
    plotting_data <- plotting_data %>% 
      filter(time == timepoint1) %>%
      dplyr::rename(meth.diff = Caste_difference) 
  }
  
  if(comparison == "timeQ"){
    facet_label <- paste("Queens: ", paste0(c(timepoint1, timepoint2), collapse = " vs "), "h", sep = "")
    plotting_data <- plotting_data %>% 
      filter(time %in% c(timepoint1, timepoint2)) %>%
      dplyr::select(Site, Queen, time, starts_with("in")) %>%
      distinct() %>% mutate(time = paste("Hour", time, sep = "")) %>%
      spread(time, Queen)
    
    plotting_data$meth.diff <- pull(plotting_data, 9) - pull(plotting_data, 8)  
  }
  
  if(comparison == "timeW"){
    facet_label <- paste("Workers: ", paste0(c(timepoint1, timepoint2), collapse = " vs "), "h", sep = "")
    plotting_data <- plotting_data %>% 
      filter(time %in% c(timepoint1, timepoint2)) %>%
      dplyr::select(Site, Worker, time, starts_with("in")) %>% 
      distinct() %>% mutate(time = paste("Hour", time, sep = "")) %>%
      spread(time, Worker)
    plotting_data$meth.diff <- pull(plotting_data, 9) - pull(plotting_data, 8)  
  }
...
plot_lm_estimates <- function(lm_estimates, xl, fill_colour){
  lm_estimates %>%
    mutate(mean_meth_diff.fit = 100 * mean_meth_diff.fit,
           mean_meth_diff.se.fit = 100 * mean_meth_diff.se.fit) %>%
    ...
    geom_point(fill = fill_colour, pch=21) + 
    coord_flip() + 
...
genomic_context_plot_caste <- 
  map_df(c(2,4,6,8), ~ make_lm_estimates("between_castes", .x, NULL)) %>% 
  plot_lm_estimates("Mean caste difference in\n% mCpGs (\u00B195% CIs)", "#b19cd9") 

genomic_context_plot_timeQ <-
  map2_df(combn(c(2,4,6,8), 2)[1,], 
          combn(c(2,4,6,8), 2)[2,], ~ make_lm_estimates("timeQ", .x, .y)) %>% 
  plot_lm_estimates("Difference in % mCpGs (mean \u00B195% CIs)", queen_colour)

genomic_context_plot_timeW <- 
  map2_df(combn(c(2,4,6,8), 2)[1,], 
          combn(c(2,4,6,8), 2)[2,], ~ make_lm_estimates("timeW", .x, .y)) %>% 
  plot_lm_estimates("Difference in % mCpGs (mean \u00B195% CIs)", worker_colour)

saveRDS(genomic_context_plot_caste, "figures/genomic_context_plot_caste.rds")
saveRDS(genomic_context_plot_timeQ, "figures/genomic_context_plot_timeQ.rds")
saveRDS(genomic_context_plot_timeW, "figures/genomic_context_plot_timeW.rds")

readRDS("figures/genomic_context_plot_caste.rds")
```

### Comparing queen-destined larvae across timepoints
```{r fig.width=6.3, fig.showtext=TRUE}
readRDS("figures/genomic_context_plot_timeQ.rds")
```
...
The heatmap shows data on the CpG sites that were measured with at least 5x coverage in all 36 samples. The version on the first tab is restricted to sites with an avearge methylation frequency of at least 75% (n = `r n_sites2` sites), while the second tab shows the data for all the methylated sites (i.e. all sites with a mean methylation frequency >2.7%; n = `r n_sites` sites). We chose to split the data this way in light of the evidence in the animation above that the greatest caste difference in methylation frequency was found among sites with high levels of methylation.

Before plotting, the data on each site was scaled to mean zero and unit standard deviation. Next, we removed batch effects (i.e. the variance due to replicates A-D) using `sva::ComBat()`. Additionally, because it is technically challenging to plot a heatmap with many thousands of rows/columns, we first clustered the sites into 100 groups of sites using k-means clustering (see `?pheatmap`). 

In the figures, the orange colour indicates that the focal sample has a methylation frequency above the average for that group of CpG sites, while purple indicates a below-average methylation frequency. The heatmap highlights that the QDL and WDL samples cluster together quite well, especially for the later time points. This indicates that QDL and WDL have disctinct methylomes, and became more distinct a few hours post-grafting. 

#### Heatmap of sites with $\ge 75$% methylation

```{r meth_sample_distance_heatmap, width = 10, height = 14, message=FALSE, warning=FALSE, fig.showtext=TRUE}
meth_sample_heatmap <- function(mean_cutoff, colour_limits, myColour){
  ...
  myBreaks <- seq(colour_limits[1], colour_limits[2], length.out=50)
  ...
  t(foc) %>%
    pheatmap(col = myColour, breaks = myBreaks, kmeans_k = 50,
...
set.seed(10)
myColour <- c(colorRampPalette(c(brewer.pal(9, "Purples")[7], "white"))(33),
    colorRampPalette(c("white", brewer.pal(9, "Oranges")[7]))(18)[-1])

heatmap75 <- meth_sample_heatmap(mean_cutoff = 0.75, colour_limits = c(-3, 1.5), myColour)

my_gtable <- heatmap75$gtable
my_gtable$grobs[[4]]$gp <- gpar(fontfamily = nice_font, fontsize=7) # x axis labels
my_gtable$grobs[[7]]$gp <- gpar(fontfamily = nice_font, fontsize=7) # Q/W legend

saveRDS(my_gtable, "figures/heatmap75.rds")
grid.arrange(my_gtable)
```

#### Heatmap of sites with $\ge 2.7$% methylation

```{r message=FALSE, warning=FALSE, width = 10, height = 14, fig.showtext=TRUE}
set.seed(10)
myColour <- c(colorRampPalette(c(brewer.pal(9, "Purples")[7], "white"))(27),
    colorRampPalette(c("white", brewer.pal(9, "Oranges")[7]))(24)[-1])

heatmap_all <- meth_sample_heatmap(mean_cutoff = 0, colour_limits = c(-3, 2.5), myColour)

my_gtable <- heatmap_all$gtable
my_gtable$grobs[[4]]$gp <- gpar(fontfamily = nice_font, fontsize=7) # x axis labels
my_gtable$grobs[[7]]$gp <- gpar(fontfamily = nice_font, fontsize=7) # Q/W legend

saveRDS(my_gtable, "figures/heatmap_all.rds")
grid.arrange(my_gtable)
```
...
bwasp_between_castes %>% dplyr::select(-site) %>% 
...
bwasp_temporal_changeQ %>% dplyr::select(-site) %>% 
...
```{r message=FALSE, warning=FALSE}
bwasp_temporal_changeW %>% dplyr::select(-site) %>% 
...
**Table XX:** The table shows the `r prettyNum(n_caste_windows, big.mark = ",")` 500bp windows in which BWASP detected a significant difference in methylation between queens and workers at one or more time points. There is one row per annotation.
...
window_500bp_timeQ %>%
  ...
  my_data_table()
```
**Table XX:** The table shows the `r prettyNum(n_queen_windows, big.mark = ",")` 500bp windows in which BWASP detected a significant change in methylation with time, in queen-destined larvae. There is one row per annotation.
...
window_500bp_timeW %>%
  ...
  my_data_table()
```
**Table XX:** The table shows the `r prettyNum(n_worker_windows, big.mark = ",")` 500bp windows in which BWASP detected a significant change in methylation with time, in worker-destined larvae. There is one row per annotation.



## GLM analysis of the methylome data
...
#### Total number of sites with sufficient data for the GLMs:
...
### Run one GLM per site using `base::glm()`

There are too many CpG sites to use the Bayesian approaches used elsewhere in this study in a reasonable amount of time. We therefore applied the equivalent frequentist generalised linear model, implemented using `base::glm()`.

To summarise the following code:

- These GLMs include all the samples except the four time zero samples. These samples were ommited because they do not neatly belong to either the queen- or worker-destined treatments, making it complicated to include in the model.
- Non-linear effects of `Time` were modeled using a quadratic term, allowing accelerating or deccelerating changes in % methylation with time
- For each site, we fit 6 different GLMs that we considered to be biologically plausible, and then performed model averaging based on their Akaike weights using `MuMIn::model.avg()`. All 6 models contained experimental block as a fixed effect, and optionally also contained the fixed effects time, time^2, the caste:time interaction, and the I(time^2):caste. Note that we did not fit the main effect of caste, which forces queens and workers to have the same % methylation estimate at time zero (which we know must be the case due to the experimental design)
...
- After fitting the models and averaging them, we predicted the % methylation of the site in queens and workers at each time point, and calculated the caste difference, its 95% CIs, and the p-value for the null hypothesis that the caste difference was zero using bootstrapping.
- Sites were re-analysed using the Bayesian models if A) the p-value for the caste difference was p < 0.00001 at one or more time points in this GLM analysis, and/or B) the site was statistically significant in the BWASP results mentioned above (FDR-corrected p-value of <0.05).

```{r message=FALSE}
# Set up split list of methylated CpG sites, and count the number of chunks of sites
split_methylated_sites <- split(methylated_sites, ceiling(seq_along(methylated_sites)/1000))
n_chunks <- length(split_methylated_sites)

############## Create helper functions
# Calculate AIC weight from delta-AIC values
# The sum of the weights for all models containing a given predictor gives its "variable importance" 
get_aic_weight <- function(aic_deltas){
  exp(-0.5 * aic_deltas) / sum(exp(-0.5 * aic_deltas))
}

# inverse logit to convert from the logit link scale to a proportion
inv_logit <- function(x) 1 / (1 + exp(-x))

# run a binomial GLM given a formula and some data
run_glm <- function(my_formula, dat) {
  glm(as.formula(my_formula), data = dat, family = "binomial")
}

# Get random normals for nSamples, nBoots times, for each value in a vector of means and SDs
# means and SDs should be the same length. Returns an array
# Used below for bootstrapping the caste difference in methylation rate at each time point
vectorised_rnorm <- function(nSamples, nBoots, means, SDs) {
  m <- length(means)
  aperm(array(rnorm(nSamples * m * nBoots, means, SDs), 
              c(m, nSamples, nBoots)), c(2,1,3))
}

# List of GLM models to compare for each site:
# Note that there is no main effect of caste, so the castes are constrained to have the same methylation at time 0 
formulae <- c(
  "cbind(numC, numT) ~ rep",
  "cbind(numC, numT) ~ rep + time",
  "cbind(numC, numT) ~ rep + time + I(time^2)",
  "cbind(numC, numT) ~ rep + caste:time",
  "cbind(numC, numT) ~ rep + caste:time + I(time^2)",
  "cbind(numC, numT) ~ rep + caste:time + I(time^2) + I(time^2):caste"
)


# Define a function to run the 6 GLMs on each site in a chunk, 
# average the models, and save key results. The function either uses the real data, or shuffles them
model_multiple_sites <- function(chunk_number, 
                                 vector_of_chunks, 
                                 random_caste, 
                                 make_plot_data = FALSE){
  ...
  focal_chunk <- by_site %>%  
    ...
    dplyr::select(sample, site, numC, numT, caste, time, rep) %>%
    distinct() %>%
    collect() 
  
  # Internal function to model one site using GLM, and compare models by AICc
  do_one_site <- function(focal_site, data_chunk, nboots = 10000){
    
    focal_site_data <- data_chunk %>%
      filter(site == focal_site)

    models <- lapply(formulae, run_glm, dat = focal_site_data)
    
    AICc_table <- data.frame(
      Site = focal_site,
      formula = str_remove_all(formulae, "cbind[(]numC, numT[)] ~ "),
      AICc(models[[1]], models[[2]], models[[3]], models[[4]], models[[5]], models[[6]])
    ) %>% as_tibble() %>% 
      arrange(AICc) %>%
      mutate(delta = AICc - AICc[1],
             weight = get_aic_weight(delta))

    # Predict new data and calculate caste difference, using model average parameter estimates
    new <- expand_grid(caste = c("Queen", "Worker"), 
                       time = c(2,4,6,8), 
                       rep = c("A", "B", "C", "D"))
  
    # Note: predict on the link scale, not data scale
    predictions_with_se <-
      data.frame(new,
                 predicted_prop_meth = predict(
                   model.avg(models), backtransform = FALSE,
                   newdata = new, se.fit = TRUE)) %>%
      group_by(caste, time) %>%
      summarise(predicted_prop_meth.fit = mean(predicted_prop_meth.fit),
                predicted_prop_meth.se.fit = mean(predicted_prop_meth.se.fit), .groups = "drop")

    # Get the caste mean prop. methylation (note back-transform from the link scale)
    caste_means <- predictions_with_se %>% 
      dplyr::select(-predicted_prop_meth.se.fit) %>% 
      spread(caste, predicted_prop_meth.fit) %>%
      mutate(Queen = inv_logit(Queen),
             Worker = inv_logit(Worker),
             Caste_difference = Queen - Worker)
    
    # Find the caste differences (and their SE and 95% CIs) by bootstrapping
    # Generate 4 Q and 4 W samples nboots times, and use to find SE/CIs of the caste difference
    # Do this 4 times, separately for times 2,4,6,8
    # 8 cols (q2 to q8, then w2 to w8), nBoots rows, giving bootstrapped means for 4 samples
    boot_means <- vectorised_rnorm(nSamples = 4, nBoots = nboots, 
                          means = predictions_with_se$predicted_prop_meth.fit, 
                          SDs = predictions_with_se$predicted_prop_meth.se.fit) %>%
    apply(2, colMeans)
    
    boot_caste_diff <- matrix(numeric(4*nboots), ncol = 4, nrow = nboots)
    
    for(i in 1:4){
      boot_caste_diff[,i] <- inv_logit(boot_means[,i]) - inv_logit(boot_means[,i+4])
    }
    
    # Helper code to summarise some bootstraps into SE and 95% CIs, and p
    my_summary <- list(
      SE = ~ sd(.x),
      lower95 = ~ as.numeric(quantile(.x, probs = 0.025)),
      upper95 = ~ as.numeric(quantile(.x, probs = 0.975)),
      p = ~ 1 - max(c(sum(.x < 0), sum(.x > 0))) / nboots
    )
    ...
    caste_diff <- 
      ...
      left_join(
        boot_caste_diff %>%
          as.data.frame() %>%
          dplyr::rename(`2` = V1, `4` = V2, `6` = V3, `8` = V4) %>%
          summarise(across(everything(), my_summary)) %>%
          gather() %>%
          mutate(time = as.numeric(str_extract(key, "[2468]")),
                 key = str_remove(key, "[2468]_")) %>%
          mutate(key = factor(key, c("Caste_difference", "SE", "lower95", "upper95", "p"))) %>%
          spread(key, value),
        by = "time") %>%
      mutate(Site = focal_site)
    
    list(AICc_table, caste_diff)
  }
  
  # Now model all the sites in the chunk, in parallel, and save this chunk's results to disk
  file_path <- "output/meth_sites_frequentist_GLMs/chunk_{chunk_number}.rds"
#  plan("multiprocess")
  chunk_results <- lapply(focal_sites, do_one_site, data_chunk = focal_chunk)
  
  if(make_plot_data) return(chunk_results)
  
  AIC_results <- map_df(chunk_results, ~ .x[[1]]) %>%
    dplyr::select(ends_with("ite"), everything()) %>% as_tibble()
  
  caste_diff_results <- map_df(chunk_results, ~ .x[[2]]) %>%
    dplyr::select(ends_with("ite"), everything()) %>% as_tibble()
  
  list(AIC_results, caste_diff_results) %>%
    saveRDS(glue(file_path))
}

# Define function to process the GLM output that was saved to disk
process_GLM_output <- function(file_path){
  all_chunks <- 
    list.files(file_path, full.names = TRUE) %>%
    map(~ readRDS(.x))

  caste_importance <- all_chunks %>% 
    map_df(~ .x[[1]]) %>%
    filter(str_detect(formula, "caste")) %>%
    group_by(Site) %>%  # sum the weights over the models containing CASTE, i.e. "variable importance"
    summarise(caste_importance = sum(weight), .groups = "drop")
  
  time_importance <- all_chunks %>% 
    map_df(~ .x[[1]]) %>%
    filter(str_detect(formula, "time")) %>%
    group_by(Site) %>%  # sum the weights over the models containing TIME, i.e. "variable importance"
    summarise(time_importance = sum(weight), .groups = "drop")
  
  interaction_importance <- all_chunks %>% 
    map_df(~ .x[[1]]) %>%
    filter(str_detect(formula, "caste[:]time")) %>%
    group_by(Site) %>%  # sum the weights over the models containing CASTE, i.e. "variable importance"
    summarise(interaction_importance = sum(weight), .groups = "drop")
  
  importance <- caste_importance %>%
    left_join(time_importance, by = "Site") %>%
    left_join(interaction_importance, by = "Site") %>%
    arrange(-interaction_importance)
  
  output_methdiff <- "output/frequentist_GLM_meth_diff.txt"
  output_importance <- "output/frequentist_GLM_importance.txt"
  
  map_df(all_chunks, ~ .x[[2]]) %>%
     write_tsv(path = output_methdiff)
  
  importance %>% write_tsv(path = output_importance)
}

# Run the GLMs and parse the results
if(!file.exists("output/frequentist_GLM_meth_diff.txt")){
  glm_results <- lapply(1:n_chunks, model_multiple_sites, 
                        vector_of_chunks = split_methylated_sites, 
                        random_caste = random_caste, shuffle = FALSE)
  rm(glm_results)
  process_GLM_output("output/meth_sites_frequentist_GLMs", shuffle = FALSE)
} else {
  glm_results_real <- read_tsv("output/frequentist_GLM_meth_diff.txt")
}



# Find the sites that were significant in these GLMs
sig_sites_caste_frequentist_GLM <- glm_results_real %>% 
  filter(p < 0.00001) %>% pull(Site) %>% unique()

# Find the sites that were significant in the BWASP pipeline
sig_sites_caste_bwasp <- bwasp_between_castes %>%
  dplyr::select(site, seqnames, start) %>% distinct()

sig_sites_timeQ_bwasp <- bwasp_temporal_changeQ %>%
  dplyr::select(site, seqnames, start) %>% distinct()

sig_sites_timeW_bwasp <- bwasp_temporal_changeW %>%
  filter(qvalue < 0.01) %>%
  dplyr::select(site, seqnames, start) %>% distinct()

sig_sites_bwasp <- bind_rows(sig_sites_caste_bwasp, 
                             sig_sites_timeQ_bwasp, sig_sites_timeW_bwasp)
sig_sites_bwasp <- sig_sites_bwasp[complete.cases(sig_sites_bwasp), ] %>% distinct()

bwasp_seq_names <- sig_sites_bwasp$seqnames %>% unique()
bwasp_start <- sig_sites_bwasp$start %>% unique()
...
sig_sites_caste_bwasp <- tbl(db, "site_info") %>%
  ...
  filter(pasted %in% paste(sig_sites_bwasp$seqnames, sig_sites_bwasp$start)) %>%
  distinct()

# Make the list of sites to analyse in the Bayesian GLMs below
sites_for_bayes_GLM <- unique(c(sig_sites_caste_frequentist_GLM, sig_sites_caste_bwasp$site))
```
...
### Run the Bayesian GLMs

The BWASP pipeline and the previous code chunk together identified `r length(sites_for_bayes_GLM)` candidate differentially-methylated sites, which we now re-analyse using a Bayesian GLM. The Bayesian approach is more flexible, e.g. it allows us to incorporate the time 0 samples into the analysis, facilitates calculation of 95% credible intervals for metrics of interest, and facilitates regularization of the parameter estimates using so-called 'skeptical' priors (see the book/lecture series _Statistical Rethinking_ by Richard McElreath). 

We first generated multiple copies of the original dataset in which the time 0 samples were relabelled as either a queen or worker. We generated all possible combinations of caste labels in which approximately equal numbers of queen and worker labels were applied to the time 0 samples; for example, if there were data on all four $t_0$ samples for the focal site (the typical situation for most of the sites in our post-QC dataset), we relabelled the $t_0$ samples 6 different ways as follows: QQWW, QWQW, QWWQ, WQQW, WQWQ, WWQQ. We then ran a Bayesian GLM on each of these six datasets, and then averaged their parameter estimates by combining their posteriors using the `brm_multiple` function in `brms`. This procedure will tend to push the estimate of % methylation at time zero towards the same value for both castes (which we know _a priori_ must be the case due to our experimental design), by effectively making each $t_0$ sample affect the parameter estimates for both queens and workers (but without duplicating any data, which would falsely inflate the model's precision).

Each model had the same formula, namely:

`Binomial(number_methylated, Coverage) ~ caste:time + I(time^2) + I(time^2):caste + replicate`

That is, the response variable is the number of putatively methylated cytosines observed (i.e. sequencing reads that contain a C rather than a T, in the BiS-seq data) recorded, over a series of $n$ Bernoulli trials where $n$ is the coverage of the site. The model formula contains time as a continuous predictor, and caste, a 2-level factor. The effect of time is modelled using a thin plate spline smoothing term, i.e. this model is a Bayesian generalised additive model (GLM). We allowed for caste-specific changes in methylation over time by including the smoothing term to differ between castes. We used mildly regularising priors for all fixed effects, namely `normal(0, 5)`, which helps to prevent overfitting. Replicate (i.e. the 4 experimental blocks) was treated as a fixed factor.
...
site_still_to_do <- sites_for_bayes_GLM[!(sites_for_bayes_GLM %in% done_already)]
...
if(length(site_still_to_do) > 0){
 ...
 # brms_formula <- bf(numC | trials(coverage) ~ caste + s(time, by = caste, k = 4) + rep)
   brms_formula <- bf(numC | trials(coverage) ~ rep + caste:time + I(time^2) + I(time^2):caste)
  ...
  if(length(site_still_to_do) > 0){
    site_still_to_do %>%
     # future_lapply(function(focal_site) {
      lapply(function(focal_site) {
        
        focal <- by_site %>%  
          dplyr::filter(site == focal_site) %>% 
          mutate(coverage = numC + numT) %>%
        ...
        time_diff_means <- preds %>% 
          ...
          dplyr::rename(Caste = tcaste, Draw = tdraw) 
        ...
        time_comparison_summary <- function(col_name){
          ...
          time_diff_means %>%
            ...
            mutate(Time = paste0(strsplit((str_extract(col_name, "[02468]+")), 
                                          split = "")[[1]], collapse="vs")) %>%
            dplyr::select(Caste, Time, everything())
        }
...
## Results of the Bayesian GLMs

### Identifying "significant" sites {.tabset}

We label a site as significantly differentially methylated between castes/timepoints if the posterior estimate of the difference in mean methylation between castes/timepoints excludes zero with over 99.99% probability (similar to a one-tailed p-value of 0.0001). We chose this definition in an attempt to balance the risk of false positives and false negatives. 

```{r get_sig_sites_bayesian_GLMs}
...
pval_caste <- caste_difference %>%
  group_by(site) %>%
  summarise(min_p = min(p),
            .groups = "drop")

pval_time <- time_difference %>%
  group_by(site, Caste) %>%
  summarise(min_p = min(p),
            .groups = "drop") %>%
  spread(Caste, min_p) %>% 
  dplyr::rename(Queen_min_p = Queen, Worker_min_p = Worker) 
...
gene_meth_results_table <- tibble(site = sites) %>%
  ...
  left_join(pval_caste, by = "site") %>% 
  dplyr::rename(caste_pval = min_p) %>%
  left_join(pval_time, by = "site") %>% 
  dplyr::rename(timeQ_pval = Queen_min_p) %>%
  dplyr::rename(timeW_pval = Worker_min_p) %>%
  mutate(site_old = site,
         site = paste(seqnames, start, sep = ", pos:")) %>%
  dplyr::select(-seqnames, -start) %>%
  arrange(caste_pval, timeQ_pval, timeW_pval, gene_name_exon) 

saveRDS(gene_meth_results_table, "output/gene_meth_results_table.rds")

# save list of sites for later plotting (highly caste-different ones)
plot_sites <- caste_difference %>%
  group_by(site) %>%
  summarise(sum_diff = sum(abs(`Q-W_difference_percent_meth`))) %>%
  arrange(-sum_diff) %>%
  pull(site) %>% 
  head(400)
...
n_sig_sites <- readRDS("output/gene_meth_results_table.rds") %>% 
  dplyr::select(-site_old) %>% 
  filter(caste_pval <= p_thresh | timeQ_pval <= p_thresh | timeW_pval <= p_thresh) %>%
  dplyr::select(site) %>% distinct() %>% nrow()

# to count the sig loci:
n_sig_sites <- gene_meth_results_table %>%
  filter(caste_pval <= p_thresh | timeQ_pval <= p_thresh | timeW_pval <= p_thresh) %>% 
  pull(site) %>% unique() %>% length()

n_sig_sites_caste <- gene_meth_results_table %>%
  filter(caste_pval <= p_thresh) %>% pull(site) %>% unique() %>% length()

n_sig_sites_timeQ <- gene_meth_results_table %>%
  filter(timeQ_pval <= p_thresh) %>% pull(site) %>% unique() %>% length()

n_sig_sites_timeW <- gene_meth_results_table %>%
  filter(timeW_pval <= p_thresh) %>% pull(site) %>% unique() %>% length() 
```


**Table SX:** The table shows the `r n_sig_sites` sites that show statistically significant differential methylation between castes and/or time points, as measured by the Bayesian generalised additive models (some sites span multiple rows due to being annotated with multiple genomic features). The criterion for significance was that the posterior estimates of the difference between a pair of means (e.g. the means for QDL and WDL at a given time point) excluded zero with 99.99% probability or better (similar to a one-tailed $p$-value of $p < 0.0001$; the table displays 1 minus this posterior probability, for comparison with $p$-values). The other columns show the annotations for the focal site, and the full names of any genes for those annotations.

```{r}
# Criterion for significance: p < 0.0001 for at least one comparison
p_thresh <- 0.0001

readRDS("output/gene_meth_results_table.rds") %>% 
  dplyr::select(-site_old) %>% 
  filter(caste_pval <= p_thresh | timeQ_pval <= p_thresh | timeW_pval <= p_thresh) %>%
  mutate(caste_pval = round(caste_pval, 6),
         timeQ_pval = round(timeQ_pval, 6),
         timeW_pval = round(timeW_pval, 6)) %>%
  dplyr::rename(`pval (difference between castes)` = caste_pval,
                `pval (temporal change in queens)` = timeQ_pval,
                `pval (temporal change in workers)` = timeW_pval) %>%
  dplyr::select(site, starts_with("pval"), everything()) %>%
  dplyr::rename_all(~ str_replace_all(.x, "pval", "p")) %>%
  my_data_table()
```


#### List of all genes overlapped by the significant sites

```{r}
gene_list <- gene_meth_results_table %>%
  filter(caste_pval <= p_thresh | timeQ_pval <= p_thresh | timeW_pval <= p_thresh) %>%
  dplyr::select(contains("_in"), contains("_of"), -contains("gene_name")) %>%
  unlist() %>% unname() %>% na.omit() %>%
  as.character() %>% unique() 

levs <- c(sort(gene_list[!str_detect(gene_list, "LOC")]), sort(gene_list[str_detect(gene_list, "LOC")]))

tibble(gene = gene_list) %>%
  mutate(gene = factor(gene, levs)) %>%
  arrange(gene) %>%
  left_join(tbl(db, "gene_names") %>% dplyr::select(gene_symbol, gene_name) %>% collect(), by = c("gene" = "gene_symbol")) %>%
...
The following plots show some of the `r n_sig_sites_caste` CpG sites that showed a statistically significant difference between castes. For brevity, we only plot the results for the 400 sites that showed the largest absolute caste difference (summed over time points). The plot below shows 16 of the `r length(plot_sites)` significant sites; click the tabs to see results for other sites. The red colour refers to queens, blue to workers. 

```{r plot_one_site, warning=FALSE, message=FALSE}
plot_one_site <- function(focal_site, include_label = TRUE){
  ...
  x <- lm(percent_meth ~ rep, data = raw_data)$coef %>% as.numeric()
  raw_data$percent_meth[raw_data$rep == "B"] <- raw_data$percent_meth[raw_data$rep == "B"] + x[2]
  raw_data$percent_meth[raw_data$rep == "C"] <- raw_data$percent_meth[raw_data$rep == "C"] + x[3]
  raw_data$percent_meth[raw_data$rep == "D"] <- raw_data$percent_meth[raw_data$rep == "D"] + x[4]
  ...
  if(include_label) return(p + labs(title = title, subtitle = subtitle[1]))
  else return(p)
}

# plot_one_site(1956531) # Csd plot
# plot_one_site(4053130) # sex-lethal homolog
# hist-lys N meth trithorax
arrangeGrob(rbind(ggplotGrob(plot_one_site(1956531, include_label = F) + 
                               labs(subtitle = "Intron in complementary sex determiner\npos: 11775528", 
                                    y = "% mCpGs")), 
                  ggplotGrob(plot_one_site(4053130, include_label = F) + 
                               labs(subtitle = "Promoter of sex-lethal homolog\npos: 6578328",
                                    x = "Hours post-grafting", y = "% mCpGs")))) %>%
  saveRDS("figures/two_sites_for_big_meth_fig.rds")

# Make all the plots
redo <- FALSE
plot_sites_split <- split(plot_sites, ceiling(seq_along(plot_sites) / 16))
done_already <- as.numeric(str_extract(list.files("figures/bayes_sites/"), "[:digit:]+"))
if(length(list.files("figures/bayes_sites/")) < length(plot_sites_split)){
  if(!redo) plot_sites_split <- plot_sites_split[(1:length(plot_sites_split))[-done_already]]
  for(i in as.numeric(names(plot_sites_split))){
    print(i)
    ggsave(glue("figures/bayes_sites/plot{i}.png"), 
           do.call("grid.arrange", 
                   lapply(plot_sites_split[[which(as.numeric(names(plot_sites_split)) == i)]], 
                          plot_one_site)),
           height = 9, width = 9)
  }
}
...
![](https://raw.githubusercontent.com/lukeholman/Methylation_Apis/master/figures/bayes_sites/plot1.png)

### Group 2

![](https://raw.githubusercontent.com/lukeholman/Methylation_Apis/master/figures/bayes_sites/plot2.png)

### Group 3

![](https://raw.githubusercontent.com/lukeholman/Methylation_Apis/master/figures/bayes_sites/plot3.png)

### Group 4

![](https://raw.githubusercontent.com/lukeholman/Methylation_Apis/master/figures/bayes_sites/plot4.png)

### Group 5

![](https://raw.githubusercontent.com/lukeholman/Methylation_Apis/master/figures/bayes_sites/plot5.png)


### Group 6

![](https://raw.githubusercontent.com/lukeholman/Methylation_Apis/master/figures/bayes_sites/plot6.png)


### Group 7

![](https://raw.githubusercontent.com/lukeholman/Methylation_Apis/master/figures/bayes_sites/plot7.png)


### Group 8

![](https://raw.githubusercontent.com/lukeholman/Methylation_Apis/master/figures/bayes_sites/plot8.png)


### Group 9

![](https://raw.githubusercontent.com/lukeholman/Methylation_Apis/master/figures/bayes_sites/plot9.png)


### Group 10

![](https://raw.githubusercontent.com/lukeholman/Methylation_Apis/master/figures/bayes_sites/plot10.png)


### Group 11

![](https://raw.githubusercontent.com/lukeholman/Methylation_Apis/master/figures/bayes_sites/plot11.png)


### Group 12

![](https://raw.githubusercontent.com/lukeholman/Methylation_Apis/master/figures/bayes_sites/plot12.png)


### Group 13

![](https://raw.githubusercontent.com/lukeholman/Methylation_Apis/master/figures/bayes_sites/plot13.png)


### Group 14

![](https://raw.githubusercontent.com/lukeholman/Methylation_Apis/master/figures/bayes_sites/plot14.png)


### Group 15

![](https://raw.githubusercontent.com/lukeholman/Methylation_Apis/master/figures/bayes_sites/plot15.png)

### Group 16

![](https://raw.githubusercontent.com/lukeholman/Methylation_Apis/master/figures/bayes_sites/plot16.png)

### Group 17

![](https://raw.githubusercontent.com/lukeholman/Methylation_Apis/master/figures/bayes_sites/plot17.png)

### Group 18

![](https://raw.githubusercontent.com/lukeholman/Methylation_Apis/master/figures/bayes_sites/plot18.png)

### Group 19

![](https://raw.githubusercontent.com/lukeholman/Methylation_Apis/master/figures/bayes_sites/plot19.png)

### Group 20

![](https://raw.githubusercontent.com/lukeholman/Methylation_Apis/master/figures/bayes_sites/plot20.png)

### Group 21

![](https://raw.githubusercontent.com/lukeholman/Methylation_Apis/master/figures/bayes_sites/plot21.png)

### Group 22

![](https://raw.githubusercontent.com/lukeholman/Methylation_Apis/master/figures/bayes_sites/plot22.png)

### Group 23

![](https://raw.githubusercontent.com/lukeholman/Methylation_Apis/master/figures/bayes_sites/plot23.png)

### Group 24

![](https://raw.githubusercontent.com/lukeholman/Methylation_Apis/master/figures/bayes_sites/plot24.png)

### Group 25

![](https://raw.githubusercontent.com/lukeholman/Methylation_Apis/master/figures/bayes_sites/plot25.png)



```{r make_multipanel_fig, echo=FALSE}
...
hm <- arrangeGrob(blankPlot, readRDS("figures/heatmap75.rds"), ncol = 1, heights = c(0.05,0.95))

bottom_right <- arrangeGrob(blankPlot, readRDS("figures/two_sites_for_big_meth_fig.rds"), ncol = 1, heights = c(0.05,0.95))


# 840 x 756px
ggsave("figures/composite_figure.pdf", grid.arrange(
  arrangeGrob(readRDS("figures/histo_fig1.rds"), 
              readRDS("figures/genomic_context_plot_caste.rds") + xlab(NULL), 
              nrow = 1, widths = c(0.6,0.4)),
  arrangeGrob(blankPlot, hm, blankPlot, bottom_right, 
              nrow = 1, widths = c(0.03, 0.6, 0.05, 0.32)), 
...
<!-- These plots are similar to Venn diagrams, but they can display overlaps between many more categories. The horizontal bar plot in the lower left shows the number of genes that contain 1 or more significantly differentially methylated sites, for a given pairwise comparison. The vertical bars showing the number of these genes that fall into each set intersection.  -->

<!-- The two analytical approaches tell a slightly different story. The Bayesian GAM approach finds more genes that were significantly differentially methylated in multiple comparisons, because it fits a model to the full dataset, while the BWASP approach finds fewer such intersections because it compares each part of the dataset in isolation from the rest. The BWASP approach identifies many differences in methylation between earlier and later time points in workers, while the Bayesian GAM additionally identifies caste differences at later time points (due to a difference in statistical power; the two analyses gives qualitatively similar results). -->

<!-- ### UpSet plot for the Bayesian GAM results -->

<!-- For brevity, only the 30 largest set intersections are plotted. Pairwise comparisons with p < 0.001 were scored as significant. -->
...
<!-- Pairwise comparisons with q < 0.01 were scored as significant. -->
...
<!-- The following shows the intersection of results from the previous two UpSet plots. For brevity, we have amalgamated the results such that genes significantly different between castes at one or more time points were scored as caste-specific, and genes that varied between one or more pairs of time points were scored as showing a temporal change. -->
...
To test whether the caste-specific sites differed because of changes in the queen methylome, the worker methylome, or both, we estimated the Spearman correlations between the 3 effect sizes per site (estimated using GLMs), namely the effect sizes measuring the change in % methylation between 8h QDL and 8h WDL, between 8h QDL and 2h QDL, and between 8h WDL and 2h WDL. 

We found that the difference between 8h QDL and 8h WDL was positively correlated with the change from 2-8h in QDL, indicating that part of the 8h caste difference reflected changes in QDL. Moreover, we found that the difference between 8h WDL and 2h WDL was negatively correlated with the change from 2-8h in QDL, suggesting that the 8h caste difference was also due to changes in WDL.

```{r message=FALSE, warning=FALSE}
read_tsv("output/frequentist_GLM_meth_diff.txt") %>%
  filter(time == 8) %>%
  dplyr::rename(`Queen 8h - Worker 8h`  = Caste_difference) %>%
  left_join(
    read_tsv("output/frequentist_GLM_meth_diff.txt") %>%
      group_by(Site) %>%
      summarise(`Queen 8h - Queen 2h` = Queen[time == 8] - Queen[time == 2]), by = "Site") %>%
  left_join(
    read_tsv("output/frequentist_GLM_meth_diff.txt") %>%
      group_by(Site) %>%
      summarise(`Worker 8h - Worker 2h` = Worker[time == 8] - Worker[time == 2]), by = "Site") %>%
  dplyr::select(contains("8h")) %>%
  cor(method = "spearman") %>%
  round(2) %>%
...
```{r echo=FALSE, message=FALSE, warning=FALSE}
site_count <- read_tsv("output/frequentist_GLM_meth_diff.txt") %>% 
  pull(Site) %>% unique() %>% length() %>% prettyNum(big.mark = ",")
```

The results in the following plots were calculated using GSEA tests for enrichment, implemented in the `fgsea` package. This test involves ranking genes by some statistic and then searching for GO and KEGG terms that are over-represented among genes at the top or bottom of the list. Here, we ranked the genes by the average of the difference in % methylation between 'classes' that was estimated for all the sites in the gene (defined as sites inside the exons, introns, or promoter regions of the gene). 'Classes' either means queens and workers (measured at the same time point), or different time points (measured in the same caste). We derive these estimates by model averaging across the five frequentist GLMs that were run on each of the `r site_count` sites for which sufficiant data were available. 

The output of `fgsea` includes the "normalised enrichment score" (NES) which describes how much the pathway is over-represented at the top of this list (for positive NES), or over-represented at the bottom (negative NES), and there is an associated $p$-value for each NES (calculated by resampling).

### Code to run enrichment tests
```{r}
...
if(!file.exists("output/meth_gsea_results.rds")){
  ...
  # Estimate caste difference in methylation for times 2/4/6/8 for each GENE 
  # (calculated as the median estimated caste difference across all the sites within
  # the exons, promoters, introns, and UTRs of each gene)
  gene_caste_difference <- read_tsv("output/frequentist_GLM_meth_diff.txt") %>%
    left_join(tbl(db, "site_annotations") %>% 
                dplyr::select(-exon_rank, -translated) %>%
                collect() %>% 
                nest(gene = c(exon_in, promoter_of, intron_in, fiveUTR_in, threeUTR_in)) %>% 
                dplyr::select(site, gene), 
              by = c("Site" = "site")) %>%
    mutate(gene = map(gene, ~ unlist(.x) %>% unname() %>% na.omit() %>% as.character())) %>%
    unnest(gene) %>% distinct() %>%
    group_by(gene, time) %>%
    summarise(statistic = median(Caste_difference), .groups = "drop") %>%
    arrange(-statistic) %>%
    left_join(tbl(db, "gene_names") %>% 
  ...
  gene_timeQ <- read_tsv("output/frequentist_GLM_meth_diff.txt") %>%
    group_by(Site) %>%
    summarise(statistic = Queen[time == 8] - Queen[time == 2], .groups = "drop") %>%
    left_join(tbl(db, "site_annotations") %>% 
                dplyr::select(-exon_rank, -translated) %>%
                collect() %>% 
                nest(gene = c(exon_in, promoter_of, intron_in, fiveUTR_in, threeUTR_in)) %>% 
    ...
    summarise(statistic = median(statistic), .groups = "drop") %>%
    arrange(-statistic) %>%
    distinct()
  
  gene_timeW <- read_tsv("output/frequentist_GLM_meth_diff.txt") %>%
    group_by(Site) %>%
    summarise(statistic = Worker[time == 8] - Worker[time == 2], .groups = "drop") %>%
    left_join(tbl(db, "site_annotations") %>% 
                dplyr::select(-exon_rank, -translated) %>%
                collect() %>% 
                nest(gene = c(exon_in, promoter_of, intron_in, fiveUTR_in, threeUTR_in)) %>% 
  ...
  significant <- GO_Amel_annotations_caste %>% 
    filter(pval < sig_thres) %>% pull(pathway) %>% unique()
  ...
  GO_Amel_annotations_temporalQ <- gene_timeQ %>% 
    GO_and_KEGG_gsea(keep.all = TRUE, GO_list = "bee_GO") %>% 
    mutate(Time = "Queen", Comparison = "Queen temporal change")
  
  GO_Amel_annotations_temporalW <- gene_timeW %>% 
    GO_and_KEGG_gsea(keep.all = TRUE, GO_list = "bee_GO") %>% 
  ...
  GO_Dmel_annotations_temporalQ <- gene_timeQ %>% 
    GO_and_KEGG_gsea(keep.all = TRUE, GO_list = "dros_ortho_GO") %>% 
    mutate(Time = "Queen", Comparison = "Queen temporal change")
  
  GO_Dmel_annotations_temporalW <- gene_timeW %>% 
    GO_and_KEGG_gsea(keep.all = TRUE, GO_list = "dros_ortho_GO") %>% 
    mutate(Time = "Worker", Comparison = "Worker temporal change")
  
  significant_q <- GO_Dmel_annotations_temporalQ %>% 
    filter(pval < sig_thres) %>% pull(pathway) %>% unique()
  significant_w <- GO_Dmel_annotations_temporalW %>% 
    filter(pval < sig_thres) %>% pull(pathway) %>% unique()
...
```{r eval=FALSE}
left_join(gene_timeQ, gene_timeW %>% 
            dplyr::rename(statistic2 = statistic)) %>% 
  arrange(-abs(statistic - statistic2)) %>% print(n=200)

left_join(gene_timeQ, gene_timeW %>% 
            dplyr::rename(statistic2 = statistic)) %>%
  filter(abs(statistic) > 0.1 | abs(statistic2) > 0.1) -> labs

left_join(gene_timeQ, gene_timeW %>% 
            dplyr::rename(statistic2 = statistic)) %>% 
  ggplot(aes(statistic, statistic2)) + geom_point() + 
  geom_text_repel(data = labs, aes(label = gene_name))
```



### Enrichment results for genes showing __caste differences__ in methylation

Red means that there is higher % methylation for genes annotated with the focal pathway/process in queens, while blue means higher % methylation in workers. A single asterisk indicates statistically significant enrichment, and a double asterisk indicates that this significant result remained after Benjamini-Hochberg false discovery rate correction (which is overly conservative here, since these multiple tests are not independent). 

#### Using the _D. melanogaster_ annotations {.tabset}

##### GO: Biological process {.tabset}

###### Figure
```{r fig.height=8, fig.width = 6.9}
...
GO_KEGG_results$GO_Dmel_annotations_caste %>%
  ...
  saveRDS("figures/meth_GO_for_composite_figure.rds")

(GO_KEGG_results$GO_Dmel_annotations_caste %>%
  filter(Test_type != "GO: Biological process" & term != "molecular_function") %>%
  make_pathway_figure("", "Hours post-grafting") +
     theme(text = element_text(family = nice_font))) %>%
  saveRDS("figures/meth_GO_others.rds")
...
###### Figure
```{r fig.width=6.9, fig.height = 7}
GO_KEGG_results$GO_Dmel_annotations_caste %>%
  filter(Test_type == "GO: Molecular function"  & term != "molecular_function") %>%
  make_pathway_figure("", "Hours post-grafting")
```
...
#### Using the _A. mellifera_ annotations {.tabset}

##### GO: Biological process {.tabset}

###### Figure
```{r fig.height=2.7, fig.width = 6.9}
(GO_KEGG_results$GO_Amel_annotations_caste %>%
  make_pathway_figure("", "Hours post-grafting") +
     theme(text = element_text(family = nice_font))) %>%
  saveRDS("figures/meth_all_enrichments_Amel.rds")
...
### Enrichment results for genes showing __temporal changes__ in methylation 

Orange means that the temporal change in % methylation was comparatively large and positive for genes annotated with the focal pathway/process, while purple means the change in % methylation was comparatively large and negative. A single asterisk indicates statistically significant enrichment, and a double asterisk indicates that this significant result remained after Benjamini-Hochberg false discovery rate correction (which is overly conservative here, since these multiple tests are not independent).
...
(bind_rows(GO_KEGG_results$GO_Dmel_annotations_temporalQ,
  ...
  make_pathway_figure("", "Caste", alt_colours = TRUE) +
     theme(text = element_text(family = nice_font))) %>%
  saveRDS("figures/meth_GO_temporal_Dmel.rds")

(bind_rows(GO_KEGG_results$GO_Amel_annotations_temporalQ,
          GO_KEGG_results$GO_Amel_annotations_temporalW) %>%
  make_pathway_figure("", "Caste", alt_colours = TRUE) +
     theme(text = element_text(family = nice_font))) %>%
  saveRDS("figures/meth_GO_temporal_Amel.rds")
```


###### Figure
```{r fig.height=6, fig.width = 5.3}
bind_rows(GO_KEGG_results$GO_Dmel_annotations_temporalQ,
          GO_KEGG_results$GO_Dmel_annotations_temporalW) %>%
  filter(Test_type == "GO: Biological process") %>%
  make_pathway_figure("", "Caste", alt_colours = TRUE)
...
do_GO_table <- function(GO_results){
  GO_results %>%
    ...
    dplyr::select(-ES, -log2err, -Comparison) %>%
    dplyr::select(Time, term, everything()) %>%
    dplyr::rename(Term=term, `Term type` = Test_type, ID = pathway, Caste = Time,
                  `Normalised enrichment score` = NES, 
                  `Leading edge genes` = leadingEdge,
                  `Corresponding gene names` = gene_names) %>%
    my_data_table() %>% formatSignif(c('pval', 'padj', 'Normalised enrichment score'), 2)
...
gsg <- goodSamplesGenes(network_input, verbose = 0)
...
The following code chunk fits a Bayesian multivariate model to the eigengene estimates from the co-methylation network produced by `WGCNA`. The 'eigengene' for sample $i$, module $j$, gives a measure of the methylation level of all the co-methylated genes that were assigned to module $j$, for sample $i$ relative to other samples. The eigengenes were transformed to have a mean of zero and standard deviation of 1 prior to analysis. 
...
Furthermore, we expected _a priori_ that some eigengenes will change non-linearly, for example by increasing to a plateau. We therefore used the same quadratic formula (implemented with `brms') to model the eigengenes, to allow for a non-linear (and potentially caste-specific) time effect. As before, we ran the model on different versions of the data where the $t_0$ samples were labeled as either queens or workers in each possible balanced combination then combined the posterior parameter estimates; this procedure ensures that the $t_0$ samples have an equal effect on the predicted values for queens and workers. Note that the main effect of caste is again missing from the model, which forces the queen and worker samples to have the same value at $t_0$ 

In `brms` notations (which is similar to e.g. the `lme4` and `mgcv` R packages), the model formula for each module is written as

$$ y \sim Caste:Time + I(Time^2) + I(Time^2):Caste + Replicate$$
...
brms_formula <- bf(
  paste("mvbind(",
        paste0(paste("m", 0:(j - 1), 
                     sep = ""), collapse = ","),
        ") ~ Caste:Time + I(Time^2) + I(Time^2):Caste + Rep", sep = "")    
) + set_rescor(FALSE)   
...
brms_data <- map(brms_data, 
                 ~ .x %>% mutate(Time = as.numeric(Time), 
                                 Caste = factor(Caste)))
...
means_data <- preds %>%
  ...
  summarise(stats = list(as.data.frame(brms::posterior_summary(eigengene))),
            .groups = "drop") %>%
  ungroup() %>% unnest(stats) %>% dplyr::rename(eigengene = Estimate)

# Find the posterior estimate of the median GLM fit line
...
gene_number_labels <- dplyr::rename(meth_network$colors %>% table() %>% 
  ...
  mutate(label_text = paste(n, "genes"),
         ...
         striptext = paste("m", str_extract_all(Module, "[:digit:]+"), " (", n, ")", sep = ""),
         striptext = factor(striptext, striptext),
         Module = factor(Module, mod_levels)) %>%
  dplyr::select(Module, striptext)

preds <- left_join(preds, gene_number_labels, by = "Module")
raw_data <- left_join(raw_data, gene_number_labels, by = "Module")
means_data <- left_join(means_data, gene_number_labels, by = "Module")
mean_line_data <- left_join(mean_line_data, gene_number_labels, by = "Module")
...
eigen_plot <- preds %>%
  ...
  geom_point(data = means_data, aes(fill = Caste), pch = 21, size = 1, alpha = 1, position = pd) +
  
  # geom_text(data = gene_number_labels, aes(label = label_text), 
  #           size = 2.8, hjust = 1, colour = "grey20", family = nice_font) + 
  facet_wrap(~ striptext, ncol = 5) + 
  ...
  coord_cartesian(ylim = c(-2, 2.6), xlim = c(0,8)) + 
  theme_bw() + 
  theme(legend.position = "top",
        text = element_text(family = nice_font),
        strip.background = element_rect(fill = "white", colour = "white"))

saveRDS(eigen_plot, "figures/eigengenes_meth_network_fig.rds")
eigen_plot
```
...
present_module_GO_table <- function(){
  ...
  focal %>%
    ...
    mutate(Module = paste("Module", Module, sep = ""))
}
