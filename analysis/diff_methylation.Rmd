...
# List of sites with at least 2.9% mean methylation, as measured by 4+ reads in at least 32 samples (used a lot below)
# 0.29% methylation is the highest BiS conversion failure rate we observed in any of the 36 samples, and so we use
# 10x this amount (2.9%) as a conservative threshold for identifying methylated sites for analysis
methylated_sites <- tbl(db, "site_info") %>%
  filter(mean_prop_meth >= 0.029 & n_samples >= 32) %>%
  distinct() %>% pull(site) 
...
<!-- "Robustness" stats for start of results... -->

<!-- ```{r} -->
<!-- xx <- by_site %>% -->
<!--   dplyr::select(site, caste, numC, numT) %>% -->
<!--   distinct() %>% -->
<!--   filter(site %in% methylated_sites) %>% -->
<!--   group_by(site) %>% -->
<!--   summarise(n_meth = sum(numC > 0), n_samples = n()) %>% -->
<!--   mutate(all_meth = n_meth == n_samples) %>% -->
<!--   collect() -->
...
if(!file.exists("figures/site_meth_hist/combined_animation.gif")){
  ...
  make_meth_density_plots <- function(by_site, site_type){
    ...
    site_meth_counts <- by_site %>%
      ...
      dplyr::select(site, numC, coverage, caste, time) %>%
  ...
  fig1_data <- get_fig1_data("translated") %>% 
    ...
    mutate(caste = replace(caste, caste =="t", "Time 0"),
           ...
           site_type = factor(site_type, c("Protein-coding regions", 
                                           "Promoter regions", "Introns", 
                                           "5'-UTRs", "3'-UTRs", "Intergenic regions")))
  ...
  nice_histo <- arrangeGrob(   # might need to call quartz() first to get this to run
...
genomic_context_plot_timeQ <-
  map2_df(combn(c(0,2,4,6,8), 2)[1,], 
          combn(c(0,2,4,6,8), 2)[2,], ~ make_lm_estimates("timeQ", .x, .y)) %>% 
  plot_lm_estimates("Difference in % mCpGs (mean \u00B195% CIs)", queen_colour)

genomic_context_plot_timeW <- 
  map2_df(combn(c(0,2,4,6,8), 2)[1,], 
          combn(c(0,2,4,6,8), 2)[2,], ~ make_lm_estimates("timeW", .x, .y)) %>% 
...
```{r fig.width=7, fig.showtext=TRUE}
...
meth_sample_heatmap <- function(mean_cutoff, colour_limits, myColour){
  ...
  # include all sites measured to 4x coverage in all 36 samples
  foc <- readRDS("output/meth_matrix.rds") 
  foc <- foc[,apply(foc, 2, function(x) sum(!is.na(x))) == 36]
...
#### Heatmap of sites with $\ge 2.9$% methylation
...
check_site <- function(focal_site){
  ...
  raw_data <- by_site %>%  
    ...
    distinct() %>%
    collect() %>%
    mutate(caste = replace(caste, caste == "t", "Worker")) %>%
    mutate(percent_meth = 100 * numC / (numC+numT))
  
  gave_warning(glm(cbind(numC, numT) ~ 
                     rep + caste:time + I(time^2) + I(time^2):caste, 
                   data = raw_data, family = "binomial"))
}

if(!file.exists("output/sites_to_ignore.rds")){
  methylated_sites_checked <- sapply(methylated_sites, check_site)
  sites_to_ignore <- methylated_sites[methylated_sites_checked]
  saveRDS(sites_to_ignore, "output/sites_to_ignore.rds")
}

methylated_sites_model <- methylated_sites[
  !(methylated_sites %in% readRDS("output/sites_to_ignore.rds"))]

coverage_per_site <- by_site %>%
  filter(site %in% methylated_sites_model) %>%
  mutate(coverage = numC + numT) %>%
  dplyr::select(site, sample, coverage) %>%
  group_by(site) %>%
...
Furthermore, a few of these sites (`r length(readRDS("output/sites_to_ignore.rds"))` sites in total) that passed this criterion nevertheless gave warnings when fitting models, usually because they were unmethylated the majority of samples. We removed these too.

The following graph and summary statistics shows the distribution of mean coverage per sample (averaging across the 32-36 samples) for each of the `length(methylated_sites_model)` sites analysed with the GLMs below. The average per-site coverage (combining reads across all 32-36 samples) was `r round(mean(coverage_per_site$total_coverage))`.

```{r}
coverage_per_site %>%
  pull(mean_coverage) %>% summary()

coverage_per_site %>%
  ggplot(aes(mean_coverage)) + 
  geom_histogram(bins = 200, fill = "white", colour = "black") +
  ylab("Number of sites") + xlab("Mean coverage per sample")
```


### Run one GLM per site using `base::glm()`

Here, we analyse the `r length(methylated_sites_model)` methylated sites for which we have sufficient data using one generalised linear model per site, via the function `base::glm()`.

To summarise the following code:

- These GLMs include all 36 samples where possible. Samples were omitted when the focal site was covered by <4 reads in the focal sample.
...
- For each site, we fit 6 different GLMs that we considered to be biologically plausible, and then performed model averaging based on their Akaike weights using `MuMIn::model.avg()`. All 6 models contained experimental block (`rep` in the models below) as a fixed effect, and optionally also contained the fixed effects `time`, `time^2`, the `caste:time` interaction, and the `I(time^2):caste` interaction. Note that we did not fit caste as predictor except in interactions with `time`, which forces queens and workers to have the same estimated % methylation at time zero (which we know must be the case due to our experimental design)
...
- We also saved the p-value for `caste:time` or `I(time^2):caste` (whichever was greater) from the top-ranked model, using the `car::Anova()` function to run a likelihood ratio test. We use this to score the 'significance' of caste for each site, and this value is presented in the table of results below (-log10 transformed)

```{r message=FALSE}
# Set up split list of methylated CpG sites, and count the number of chunks of sites
split_methylated_sites <- split(methylated_sites_model,
                                ceiling(seq_along(methylated_sites_model)/1000))
...
model_multiple_sites <- function(chunk_number, 
  ...
  focal_chunk <- by_site %>%  
    filter(site %in% focal_sites) %>% 
    dplyr::select(sample, site, numC, numT, caste, time, rep) %>%
    distinct() %>%
    collect() %>%
    mutate(caste = replace(caste, caste == "t", "Worker")) # t0 caste labelled as worker
  ...
  do_one_site <- function(focal_site, data_chunk, nboots = 10000){
    ...
    # Get the anova table for top-AICc model
    top_model_anova <- as.data.frame(car::Anova(models[[AICc_table$model[1]]], type = "III")) %>%
      rownames_to_column("Parameter") %>%
      mutate(Site = focal_site)
    AICc_table <- AICc_table %>% dplyr::select(-model)
    ...
    caste_means <- predictions_with_se %>% 
      ...
      mutate(Site = focal_site,
             Queen = inv_logit(Queen),
             Worker = inv_logit(Worker),
             Caste_difference = Queen - Worker) %>% 
      dplyr::select(Site, everything())
    
    list(AICc_table, caste_means, top_model_anova)
  }
  
  # Now model all the sites in the chunk, and save this chunk's results to disk
  ...
  top_model_anova <- map_df(chunk_results, ~ .x[[3]]) %>%
    dplyr::select(ends_with("ite"), everything()) %>% as_tibble()
  
  list(AIC_results, caste_diff_results, top_model_anova) %>%
...
process_GLM_output <- function(file_path){
  ...
  # P value for the most signficant parameter involving caste, from the top-AICc model
  # Only sites where the top model contained caste appear here (others assumed < 0.05)
  # The p value is either for the caste:time or caste:time^2 effect, whichever is lesser
  p_values <- all_chunks %>% 
    map_df(~ .x[[3]]) %>%
    filter(str_detect(Parameter, "caste")) %>%
    group_by(Site) %>%
    summarise(p = min(`Pr(>Chisq)`), .groups = "drop") 
  
  map_df(all_chunks, ~ .x[[2]]) %>%
    write_tsv(path = "output/frequentist_GLM_meth_diff.txt")
  
  importance %>% write_tsv(path = "output/frequentist_GLM_importance.txt")
  p_values %>% write_tsv(path = "output/frequentist_GLM_p_values.txt")
}
...
if(!file.exists("output/frequentist_GLM_meth_diff.txt")){
  ...
  rm(glm_results)
  process_GLM_output("output/meth_sites_frequentist_GLMs")
} else {
  glm_results <- read_tsv("output/frequentist_GLM_meth_diff.txt") %>%
    filter(Site %in% methylated_sites_model)
  importance <- read_tsv("output/frequentist_GLM_importance.txt") %>%
    filter(Site %in% methylated_sites_model)
  p_values <- read_tsv("output/frequentist_GLM_p_values.txt") %>%
    filter(Site %in% methylated_sites_model)
}

# Find the sites that were p<0.0001 in these GLMs
sig_sites_caste_GLM <- p_values %>% 
  filter(p < 0.0001) %>% pull(Site) %>% unique()

n_FDR_sig <- p_values %>%
  mutate(FDR_sig = ifelse(p.adjust(p) < 0.05, "*", "")) %>%
  filter(FDR_sig == "*") %>% nrow()
```

### Differentially methylated sites  {.tabset}

#### Sites showing significant differential methylation 

We label a CpG site as significantly differentially methylated between castes/timepoints if the effect of caste (as measured by the best-fitting model) was $p$ < 0.0001 (n = `r length(sig_sites_caste_GLM)` sites). Not all sites survived false discovery rate correction; those that did are labelled with an asterisk the table below (we include both since the false discovery rate correction is overly conservative here, since these tests are not independent due to correlations in methylation between sites) (n = `r n_FDR_sig` sites were signficant after FDR correction). The table shows the location of each significant site, and mentions if this site is found in parts of one or more genes. See the two other tabs for an easier-to-read list of genes that contained significant sites (either p < 0.0001 or FDR-significant).

```{r}
gene_meth_results_table <- p_values %>%
  mutate(FDR_sig = ifelse(p.adjust(p) < 0.05, "*", "")) %>%
  filter(Site %in% sig_sites_caste_GLM) %>%
  dplyr::rename(site = Site) %>%
  left_join(
    glm_results %>% group_by(Site) %>% 
      summarise(direction = ifelse(Caste_difference[which.max(abs(Caste_difference))] > 0, 
                                   "Q>W", "W>Q"), 
                difference = round(100 * abs(Caste_difference)[which.max(abs(Caste_difference))], 1),
                .groups = "drop"), 
    by = c("site" = "Site")) %>%
  arrange(p) %>%
  ...
  mutate(p = round(-1 * log10(p), 1)) %>%
  dplyr::rename(`log10 p` = p, pos = start) %>%
  rename_all(~ str_replace_all(.x, "_", " ")) %>%
  rename_all(~ str_replace_all(.x, "five", "5")) %>%
  rename_all(~ str_replace_all(.x, "three", "3")) %>%
  dplyr::select(-translated) 

# Get the list of FDR-significant sites that will be plotted below:
plot_sites <- gene_meth_results_table %>% 
  filter(`FDR sig` == "*") %>% pull(site) %>% unique()
...
#### Genes overlapped by the $p < 0.0001$ sites

```{r}
gene_list <- gene_meth_results_table %>%
  dplyr::select(direction, contains(" in"), contains(" of"), -contains("gene name")) %>%
  unlist() %>% unname() %>% na.omit() %>%
  as.character() %>% unique() 

gene_list <- gene_list[!(gene_list %in% c("Q>W", "W>Q"))]
...
#### Genes overlapped by the FDR-significant sites
...
plot_one_site <- function(focal_site, title=NULL, subtitle=NULL){
  ...
  model <- glm(cbind(numC, numT) ~ rep + caste:time + I(time^2) + I(time^2):caste, data = raw_data, family = "binomial")
  ...
  preds <- data.frame(new, percent_meth = 100 * predict(model, newdata=new,type="response")) %>% 
    as_tibble() %>%
    group_by(caste, time) %>%
    summarise(percent_meth = mean(percent_meth), .groups = "drop")
  ...
  seq_pos <- tbl(db, "site_info") %>%
    filter(site == focal_site) %>%
    dplyr::select(seqnames, start) %>%
    collect() 
  seq <- seq_pos %>% pull(seqnames)
  pos <- seq_pos %>% pull(start)
  if(is.null(subtitle)) subtitle <- glue::glue("seq: {seq}, pos: {pos}")
  if(is.null(title) & length(foc_site_genes)>0) {
    title <- tbl(db, "gene_names") %>% filter(gene_symbol %in% foc_site_genes) %>% pull(gene_name)
    if(length(title)>1) sort(title)[1] 
  }
  
  if(is.null(title) & length(foc_site_genes)==0) title <- " "
  
  title <-  str_wrap(title, width = 35)
...
arrangeGrob(rbind(ggplotGrob(plot_one_site(1956531, title = "D. Intron in csd") +
                               labs(y = "% mCpGs")),
                  ggplotGrob(plot_one_site(4053130, title = "Promoter of sex-lethal homolog") +
                               labs(x = "Hours post-grafting", y = "% mCpGs")))) %>%
  saveRDS("figures/two_sites_for_big_meth_fig.rds")

redo <- F  # Make all the plots and save to disk for later display
plot_sites_split <- split(plot_sites, ceiling(seq_along(plot_sites) / 16))
done_already <- as.numeric(str_extract(list.files("figures/meth_sites/"), "[:digit:]+"))
if(length(list.files("figures/meth_sites/")) < length(plot_sites_split)){
  ...
  for(i in as.numeric(names(plot_sites_split))){
    print(i)
    ggsave(glue("figures/meth_sites/plot{i}.png"),
...
![](https://raw.githubusercontent.com/lukeholman/Methylation_Apis/master/figures/meth_sites/plot1.png)

### Group 2

![](https://raw.githubusercontent.com/lukeholman/Methylation_Apis/master/figures/meth_sites/plot2.png)

### Group 3

![](https://raw.githubusercontent.com/lukeholman/Methylation_Apis/master/figures/meth_sites/plot3.png)

### Group 4

![](https://raw.githubusercontent.com/lukeholman/Methylation_Apis/master/figures/meth_sites/plot4.png)

### Group 5

![](https://raw.githubusercontent.com/lukeholman/Methylation_Apis/master/figures/meth_sites/plot5.png)


### Group 6

![](https://raw.githubusercontent.com/lukeholman/Methylation_Apis/master/figures/meth_sites/plot6.png)


### Group 7

![](https://raw.githubusercontent.com/lukeholman/Methylation_Apis/master/figures/meth_sites/plot7.png)


### Group 8

![](https://raw.githubusercontent.com/lukeholman/Methylation_Apis/master/figures/meth_sites/plot8.png)


### Group 9

![](https://raw.githubusercontent.com/lukeholman/Methylation_Apis/master/figures/meth_sites/plot9.png)


### Group 10

![](https://raw.githubusercontent.com/lukeholman/Methylation_Apis/master/figures/meth_sites/plot10.png)


### Group 11

![](https://raw.githubusercontent.com/lukeholman/Methylation_Apis/master/figures/meth_sites/plot11.png)


### Group 12

![](https://raw.githubusercontent.com/lukeholman/Methylation_Apis/master/figures/meth_sites/plot12.png)


### Group 13

![](https://raw.githubusercontent.com/lukeholman/Methylation_Apis/master/figures/meth_sites/plot13.png)


### Group 14

![](https://raw.githubusercontent.com/lukeholman/Methylation_Apis/master/figures/meth_sites/plot14.png)


### Group 15

![](https://raw.githubusercontent.com/lukeholman/Methylation_Apis/master/figures/meth_sites/plot15.png)

### Group 16

![](https://raw.githubusercontent.com/lukeholman/Methylation_Apis/master/figures/meth_sites/plot16.png)

### Group 17

![](https://raw.githubusercontent.com/lukeholman/Methylation_Apis/master/figures/meth_sites/plot17.png)

### Group 18

![](https://raw.githubusercontent.com/lukeholman/Methylation_Apis/master/figures/meth_sites/plot18.png)

### Group 19

![](https://raw.githubusercontent.com/lukeholman/Methylation_Apis/master/figures/meth_sites/plot19.png)

### Group 20

![](https://raw.githubusercontent.com/lukeholman/Methylation_Apis/master/figures/meth_sites/plot20.png)

### Group 21

![](https://raw.githubusercontent.com/lukeholman/Methylation_Apis/master/figures/meth_sites/plot21.png)

### Group 22

![](https://raw.githubusercontent.com/lukeholman/Methylation_Apis/master/figures/meth_sites/plot22.png)

### Group 23

![](https://raw.githubusercontent.com/lukeholman/Methylation_Apis/master/figures/meth_sites/plot23.png)
...
# 840 x 756px
ggsave("figures/composite_figure.pdf", 
       grid.arrange(
         arrangeGrob(readRDS("figures/histo_fig1.rds"), 
                     readRDS("figures/genomic_context_plot_caste.rds") + 
                       xlab(NULL) +  labs(subtitle = "B."), 
...
To test whether the caste-specific sites differed because of changes in the queen methylome, the worker methylome, or both, we estimated the Spearman correlations between the 3 effect sizes per site (estimated using GLMs), namely the effect sizes measuring the change in % methylation between 8h QDL and 8h WDL, between 8h QDL and 0h QDL, and between 8h WDL and 0h WDL. 

We found that the difference between 8h QDL and 8h WDL was positively correlated with the change from 0-8h in QDL, indicating that part of the 8h caste difference reflected changes in QDL. Moreover, we found that the difference between 8h WDL and 0h WDL was negatively correlated with the change from 0-8h in QDL, suggesting that the 8h caste difference was also due to changes in WDL.

```{r message=FALSE, warning=FALSE}
cor_dat <- read_tsv("output/frequentist_GLM_meth_diff.txt") %>%
  filter(time == 8) %>%
  dplyr::rename(`Queen 8h - Worker 8h`  = Caste_difference) %>%
  left_join(
    read_tsv("output/frequentist_GLM_meth_diff.txt") %>%
      group_by(Site) %>%
      summarise(`Queen 8h - Queen 0h` = Queen[time == 8] - Queen[time == 0]), by = "Site") %>%
  left_join(
    read_tsv("output/frequentist_GLM_meth_diff.txt") %>%
      group_by(Site) %>%
      summarise(`Worker 8h - Worker 0h` = Worker[time == 8] - Worker[time == 0]), by = "Site") %>%
  dplyr::select(contains("8h")) 

cor.test(cor_dat$`Queen 8h - Worker 8h`, cor_dat$`Queen 8h - Queen 0h`, method = "spearman")
cor.test(cor_dat$`Queen 8h - Worker 8h`, cor_dat$`Worker 8h - Worker 0h`, method = "spearman")
cor.test(cor_dat$`Queen 8h - Queen 0h`, cor_dat$`Worker 8h - Worker 0h`, method = "spearman")

```


## Plot genomic locations of (caste-specific) methylation

__Figure__: The following plot shows that DNA methylation is found all 16 of the largest contigs (putative chromosomes), and that sites showing caste-specific methylation are widely distributed across the methylome. The solid black line shows the density of sites with >50% DNA methylation, which closely correlates with the density of genes (shown by the dashed black line). The purple line shows the density of sites that we identified as having significant caste-specific methylation ($p$ < 0.0001), which also follows the overall distribution of genes with few exceptions. The coloured rug plot at the bottom marks methylated sites (grey) and differentially methylated sites (red: QDL-biased, and blue: WDL-biased). 

```{r warning=F, message=F}
source("code/add_gene_exon_columns.R")

heavily_methylated_sites <- tbl(db, "site_info") %>%
  filter(mean_prop_meth >= 0.5 & n_samples >= 32) %>%
  distinct() %>% pull(site) 

hits <- gene_meth_results_table %>% 
  filter(substr(seqnames,1,2) == "NC")

methylome <- tbl(db, "site_info") %>%
  filter(site %in% heavily_methylated_sites) %>%
  dplyr::select(seqnames, start) %>%
  collect() %>% dplyr::rename(pos = start)  %>%
  filter(seqnames %in% gene_meth_results_table$seqnames)  %>% 
  filter(substr(seqnames,1,2) == "NC") 

p <- as.data.frame(all_transcribed_regions) %>% 
  mutate(gene_id = 1:n()) %>%
  filter(seqnames %in% gene_meth_results_table$seqnames)  %>% 
  filter(substr(seqnames,1,2) == "NC") %>%
  group_by(gene_id) %>%
  summarise(seqnames = seqnames[1], pos = mean(start[1], end[1]))  %>%
  ggplot(aes(x = pos/10^6)) + 
  geom_density(linetype = 3) +
  facet_wrap(~ seqnames, scales= "free") +
  geom_rug(data = methylome, alpha = 0.01) + 
  geom_density(data = methylome, colour = "black") +
  geom_density(data = hits, colour = "#b19cd9") +
  geom_rug(data = hits, aes(colour = direction)) + 
  scale_colour_manual(values = c(queen_colour, worker_colour)) + 
  xlab("Position (megabases)") + ylab("Density") + 
  theme_bw() + 
  theme(strip.background = element_blank(),
        text = element_text(family = nice_font),
        legend.position = "none")

saveRDS(p, "figures/genomic_methyl_distribution.rds")
p
```
...
if(!file.exists("output/meth_gsea_results.rds")){
  ...
  # (calculated as the median estimated caste difference across all high methylation CpG sites (>40%) within
  ...
  high_meth_sites <- gene_caste_difference %>% filter(time == 8) %>% 
    dplyr::select(Site, Queen, Worker) %>% 
    distinct() %>% filter(Queen > 0.4 | Worker > 0.4) %>% 
    pull(Site)
  
  gene_caste_difference <- gene_caste_difference %>%
    filter(Site %in% high_meth_sites) %>% 
    group_by(gene, time) %>%
    summarise(statistic = max(Caste_difference), .groups = "drop") %>%
    arrange(-statistic) %>%
    left_join(tbl(db, "gene_names") %>% 
  ...
  gene_timeQ <- read_tsv("output/frequentist_GLM_meth_diff.txt") %>%
    filter(Site %in% high_meth_sites) %>% 
    group_by(Site) %>%
    summarise(statistic = Queen[time == 8] - Queen[time == 0], .groups = "drop") %>%
    left_join(tbl(db, "site_annotations") %>% 
                dplyr::select(-exon_rank, -translated) %>%
                collect() %>% 
                nest(gene = c(exon_in, promoter_of, intron_in, fiveUTR_in, threeUTR_in)) %>% 
  ...
  gene_timeW <- read_tsv("output/frequentist_GLM_meth_diff.txt") %>%
    filter(Site %in% high_meth_sites) %>% 
    group_by(Site) %>%
    summarise(statistic = Worker[time == 8] - Worker[time == 0], .groups = "drop") %>%
    left_join(tbl(db, "site_annotations") %>% 
                dplyr::select(-exon_rank, -translated) %>%
                collect() %>% 
                nest(gene = c(exon_in, promoter_of, intron_in, fiveUTR_in, threeUTR_in)) %>% 
  ...
  GO_Amel_annotations_caste <- rbind(
    gene_caste_difference %>% filter(time == 2) %>% 
      GO_and_KEGG_gsea(keep.all = TRUE, GO_list = "bee_GO") %>% 
      mutate(Time = 2, Comparison = "Caste"),
    gene_caste_difference %>% filter(time == 4) %>% 
      GO_and_KEGG_gsea(keep.all = TRUE, GO_list = "bee_GO") %>% 
      mutate(Time = 4, Comparison = "Caste"),
    gene_caste_difference %>% filter(time == 6) %>% 
      GO_and_KEGG_gsea(keep.all = TRUE, GO_list = "bee_GO") %>% 
      mutate(Time = 6, Comparison = "Caste"),
    gene_caste_difference %>% filter(time == 8) %>% 
      GO_and_KEGG_gsea(keep.all = TRUE, GO_list = "bee_GO") %>% 
...
# Filter out genes with <2.9% methylation
network_input <- network_input[, colMeans(network_input) < 2.9]
...
To create a co-methylation network, we used the % methylation values per gene (i.e. pooling across all the CpG sites in each gene) that were inferred by the BWASP pipeline. We analysed all genes that showed non-zero variability in % methylation across samples, which had >=2.9% mean methylation frequency, and which were methylated in >=30 samples. The dataset used to build the co-methylation network thus contained 36 samples and `r ncol(network_input)` genes.
...
brms_data <- meth_eigengenes %>%
  mutate(Caste = replace(as.character(Caste), Time == 0, "w")) %>%
...
eigen_plot <- preds %>%
  ...
  coord_cartesian(ylim = c(-2, 2.7), xlim = c(0,8)) + 
  theme_bw() + 
...
present_module_GO_table() %>%
  write_csv("output/module_GO_table_meth.csv")
```
