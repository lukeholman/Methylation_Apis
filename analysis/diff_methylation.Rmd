...
The following uses the results of the frequentist GAMs generated below. For each site, we fit 5 GAMs and combined them using model averaging to get predictions of the mean % methylation for QDL and WDL at each time point. We then calculated the difference in means for each kind of site (i.e. the effect size), for various pairwise comparisons between castes and time points. 

We then used a linear model to estimate the average effect size for sites of each genomic context provided in the .gff file of genome annotations (e.g. exons, introns, UTRs). This was accomplished by running a linear model with the following formula:
...
```{r site_type_plots, message=FALSE, warning=FALSE, fig.width=5.3, fig.showtext=TRUE}
#if(!file.exists("figures/genomic_context_plot_timeW.rds")){
  ...
  #   }
  #   
  #   model <- lm(Caste_difference ~ in_coding_region + in_promoter + in_intron + in_5UTR + in_3UTR, 
  #               data =  plotting_data)
  #   
...
foc_sites <- read_tsv("output/frequentist_GAM_meth_diff.txt") %>% pull(Site) %>% unique()

plotting_data <- read_tsv("output/frequentist_GAM_meth_diff.txt") %>%
...
# args should be like "timeQ", 2, 6 for 2h vs 6h comp
make_lm_estimates <- function(comparison, timepoint1, timepoint2){  
  
  if(comparison == "between_castes"){
    facet_label <- paste("Caste difference (", timepoint1, "h)", sep = "")
    plotting_data <- plotting_data %>% 
...
genomic_context_plot_caste <- 
  ...
  plot_lm_estimates("Difference in % methylation\n(Mean \u00B1 95% CIs; positive means Queens > Workers)")

genomic_context_plot_timeQ <-
  map2_df(combn(c(2,4,6,8), 2)[1,], 
          combn(c(2,4,6,8), 2)[2,], ~ make_lm_estimates("timeQ", .x, .y)) %>% 
  plot_lm_estimates("Difference in % methylation\n(Mean \u00B1 95% CIs; positive means methylation increases over time)")
...
meth_sample_heatmap <- function(mean_cutoff, colour_limits){
  ...
  paletteLength <- 50
  myColor <- colorRampPalette(c(brewer.pal(9, "Purples")[7], 
                                "white", brewer.pal(9, "Oranges")[7]))(paletteLength)
...
if(!file.exists("output/list_of_DMS_frequentist_GAM.txt")){
  ...
  model_multiple_sites <- function(chunk_number, vector_of_chunks){
    ...
    do_one_site <- function(focal_site, data_chunk){
      ...
      m1 <- gam(cbind(numC, numT) ~ caste + s(time, by = caste, k = 4) + rep, data = focal_site_data, family = "binomial")
      m2 <- gam(cbind(numC, numT) ~ caste + s(time,  k = 4) + rep, data = focal_site_data, family = "binomial")
      m3 <- gam(cbind(numC, numT) ~ caste + rep, data = focal_site_data, family = "binomial")
      m4 <- gam(cbind(numC, numT) ~ s(time,  k = 4) + rep, data = focal_site_data, family = "binomial")
      m5 <- gam(cbind(numC, numT) ~ rep, data = focal_site_data, family = "binomial")
      
      new <- expand_grid(caste = c("Queen", "Worker"), time = c(2,4,6,8), rep = "A")
      
      model_averages <- 
        data.frame(dplyr::select(new, -rep), 
                   predicted_prop_meth = predict(model.avg(m1, m2, m3, m4, m5), 
                                                 newdata = new, type = "link", backtransform = TRUE)) %>%
        spread(caste, predicted_prop_meth) %>%
        mutate(Caste_difference = Queen - Worker)
      
      
      # Make AIC model comparison table on the 5 candidate models
      AIC_table <- tibble(Site = focal_site, 
                          Model = c("caste + s(time, by = caste)", 
                                    ...
                                    "Replicate only")) %>%
        bind_cols(
          AICc(m1, m2, m3, m4, m5) %>% as.data.frame() %>% as_tibble()
        ) %>% arrange(AICc) %>% mutate(delta = AICc - AICc[1])
      
      list(model_averages, AIC_table)
    }
  ...
  gam_results <- lapply(1:n_chunks, model_multiple_sites, split_methylated_sites)
  ...
  frequentist_GAM_meth_diff <- list.files("data/meth_sites_GAMs", full.names = TRUE) %>%
    map_df(~ readRDS(.x) %>% map_df(~ {
      data.frame(Site = .x[[2]]$Site[1], .x[[1]])
      })) %>% as_tibble() 
  
  caste_importance <- list.files("data/meth_sites_GAMs", full.names = TRUE) %>%
    map_df(~ readRDS(.x) %>% map_df(~ .x[[2]])) %>%
    group_by(Site) %>%
    mutate(weight = get_aic_weight(delta)) %>% 
    ungroup() %>%
    filter(Model %in% c("caste", "caste + s(time)", "caste + s(time, by = caste)")) %>%
    group_by(Site) %>%  # sum the weights over the 3 models containining CASTE, i.e. "variable importance"
  ...
  interaction_importance <- list.files("data/meth_sites_GAMs", full.names = TRUE) %>%
    ...
    dplyr::rename(interaction_importance = weight) %>%
    dplyr::select(Site, interaction_importance)
  ...
  write_tsv(frequentist_GAM_meth_diff, "output/frequentist_GAM_meth_diff.txt")
...
# plot_one_site(1956531) # nice Csd plot
...
```{r echo=FALSE, message=FALSE, warning=FALSE}
site_count <- read_tsv("output/frequentist_GAM_meth_diff.txt") %>% 
...
if(!file.exists("output/meth_gsea_results.rds")){
  ...
  gene_caste_difference <- read_tsv("output/frequentist_GAM_meth_diff.txt") %>%
    left_join(tbl(db, "site_annotations") %>% 
                filter(site %in% !!importance$Site) %>% 
    ...
    group_by(gene, time) %>%
    summarise(statistic = mean(Caste_difference), .groups = "drop") %>%
    arrange(-statistic) %>%
    left_join(tbl(db, "gene_names") %>% 
  ...
  gene_timeQ <- read_tsv("output/frequentist_GAM_meth_diff.txt") %>%
    group_by(Site) %>%
    summarise(statistic = Queen[4] - Queen[1]) %>%
    left_join(tbl(db, "site_annotations") %>% 
                filter(site %in% !!importance$Site) %>% 
                dplyr::select(-exon_rank, -translated) %>%
                collect() %>% 
    ...
    summarise(statistic = mean(statistic), .groups = "drop") %>%
    arrange(-statistic) %>%
    distinct()
  
  gene_timeW <- read_tsv("output/frequentist_GAM_meth_diff.txt") %>%
    group_by(Site) %>%
    summarise(statistic = Worker[4] - Worker[1]) %>%
    left_join(tbl(db, "site_annotations") %>% 
                filter(site %in% !!importance$Site) %>% 
                dplyr::select(-exon_rank, -translated) %>%
                collect() %>% 
