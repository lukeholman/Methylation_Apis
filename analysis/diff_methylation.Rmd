...
# Make the list of sites to analyse in the Bayesian GAMs below
sites_for_bayes_GAM <- unique(c(sig_sites_caste_frequentist_GLM, sig_sites_caste_bwasp$site))
```
...
# save list of sites for later plotting (highly sig. caste ones)
...
The following plots show the `r length(plot_sites)` CpG sites that showed a highly statistically significant difference between castes and/or time points (defined as a proportion of at least $(1- 10^{-8})$ posterior probability of the differences in caste-specific means lies to one side of zero, for one or more time points). The total number of sites tested was `r nrow(importance)`. Of these, `r n_sig_sites_caste` were differentially methylated between castes with $p < 0.0001$, `r n_sig_sites_timeQ` showed significant temporal variation in queens, and `r n_sig_sites_timeW` showed significant temporal variation in workers. For brevity we only plot the most strongly significant sites, and focus only on sites showing a caste difference.

The plot below shows 16 of the `r length(plot_sites)` significant sites; click the tabs to see the remainder. The red colour refers to queens, blue to workers. 
...
if(length(list.files("figures/bayes_sites/")) < length(plot_sites_split)){
  ...
  for(i in as.numeric(names(plot_sites_split))){
    ...
    ggsave(glue("figures/bayes_sites/plot{i}.png"), 
           do.call("grid.arrange", lapply(plot_sites_split[[which(as.numeric(names(plot_sites_split)) == i)]], plot_one_site)),
           height = 9, width = 9)
  }
}
```
...
To test whether the caste-specific sites differed because of changes in the queen methylome, the worker methylome, or both, we estimated the Spearman correlations between the 3 effect sizes per site (estimated using GAMs), namely the effect sizes measuring the change in % methylation between 8h QDL and 8h WDL, between 8h QDL and 2h QDL, and between 8h WDL and 2h WDL. 
