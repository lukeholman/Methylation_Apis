...
# List of sites with at least 2.7% mean methylation, as measured by 4+ reads in at least 32 samples (used a lot below)
...
methylated_sites_model <- methylated_sites[!(methylated_sites %in% readRDS("output/sites_to_ignore.rds"))]
...
model_multiple_sites <- function(chunk_number, 
  ...
  do_one_site <- function(focal_site, data_chunk, nboots = 10000){
    ...
    # caste_diff <- 
    #   caste_means %>% 
    #   dplyr::select(time, Queen, Worker) %>%
    #   left_join(
    #     boot_caste_diff %>%
    #       as.data.frame() %>%
...
#   filter(qvalue < 0.05) %>%
#   pull(site) %>% unique() %>% na.omit() %>% as.numeric()
# 
# # Make the list of sites to analyse in the Bayesian GLMs below
# sites_for_bayes_GLM <- unique(c(sig_sites_caste_GLM, sig_sites_caste_bwasp))
```
...
gene_meth_results_table <- p_values %>%
  ...
  left_join(glm_results %>% group_by(Site) %>% 
...
arrangeGrob(rbind(ggplotGrob(plot_one_site(1956531, title = "Intron in Csd") +
                               labs(y = "% mCpGs")),
