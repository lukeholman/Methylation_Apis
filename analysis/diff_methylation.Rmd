...
# sites_for_bayes_GLM <- unique(c(sig_sites_caste_frequentist_GLM, sig_sites_caste_bwasp))
```
...
We label a CpG site as significantly differentially methylated between castes/timepoints if the effect of caste (as measured by the best-fitting model) was $p$ < 0.0001. Not all sites survived false discovery rate correction; those that did are labelled with an asterisk the table below (we include both since the false discovery rate correction is overly conservative here, since these tests are not independent due to correlations in methylation between sites). The table shows the location of each significant site, and mentioned if it is found in parts of one or more genes. 

```{r}
# Find the sites that were p<0.0001 in these GLMs
sig_sites_caste_frequentist_GLM <- p_values %>% 
  filter(p < 0.0001) %>% pull(Site) %>% unique()

gene_meth_results_table <- p_values %>%
  mutate(FDR_sig = ifelse(p.adjust(p) < 0.05, "*", "")) %>%
  filter(Site %in% sig_sites_caste_frequentist_GLM) %>%
...
__Figure__: The following plot shows that DNA methylation is found all 16 of the largest contigs (putative chromosomes), and that sites showing caste-specific methylation are widely distributed across the methylome. The solid black line shows the density of sites with >50% DNA methylation, which closely correlates with the density of genes (shown by the dashed black line). The purple line shows the density of sites that we identified as having significant caste-specific methylation (FDR-corrected $p$ < 0.05), which also follows the overall distribution of genes with few exceptions. The coloured rug plot at the bottom marks methylated sites (grey) and differentially methylated sites (red: QDL-biased, and blue: WDL-biased). 
...
as.data.frame(all_transcribed_regions) %>% 
  mutate(gene_id = 1:n()) %>%
  filter(seqnames %in% gene_meth_results_table$seqnames)  %>% 
  filter(substr(seqnames,1,2) == "NC") %>%
  group_by(gene_id) %>%
