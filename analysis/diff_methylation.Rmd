...
```{r meth_sample_distance_heatmap, width = 11, height = 12, message=FALSE, warning=FALSE}
...
```{r message=FALSE, warning=FALSE, width = 11, height = 12}
...
if(!file.exists("output/meth_gsea_results.rds")){
  ...
  significant <- GO_Dmel_annotations_caste %>% filter(pval < 0.05) %>% 
    pull(pathway) %>% unique()
...
```{r eval=FALSE}
left_join(gene_timeQ, gene_timeW %>% dplyr::rename(statistic2 = statistic)) %>% arrange(-abs(statistic - statistic2)) %>% print(n=200)

left_join(gene_timeQ, gene_timeW %>% dplyr::rename(statistic2 = statistic)) %>%
