...
sig_sites_caste <- caste_difference %>%
  group_by(site) %>%
  summarise(sig = ifelse(any(p < 0.0001), TRUE, FALSE), 
            .groups = "drop") %>%
...
saveRDS(list(sig_sites_caste = sig_sites_caste, 
             sig_sites_time_queen = sig_sites_time_queen, 
             sig_sites_time_worker = sig_sites_time_worker), 
        "output/sig_sites_bayesianGAM.rds")
...
# save list of sites for later plotting (highly significant ones)
