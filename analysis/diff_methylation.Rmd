...
The below plot uses the gene-level estimates of % methylation for each sample provided by BWASP (these estimates seek the average % methylation of all the sites that map to the focal gene). The great majority of genes show higher % methylation (averaged over time points and samples) in worker-destined larvae, suggesting that the large numbers of genes are methylated in response to the rearing conditions experienced by workers, and/or are *de*-methylated in queen-destined larvae (later results suggest the former is more common). 
...
# Ask Claire: Why does this give different results?
# vroom::vroom("data/methyl_diff_results/methyl_diff_results.tsv") %>%
#   filter(Genes %in% dat$name) %>%
#   filter(comparison == "queen6.vs.worker6") %>%
#   group_by(Genes) %>% summarise(diff = mean(meth.diff)) %>%
#   ggplot(aes(diff)) +
#   geom_vline(xintercept = 0, linetype = 2, colour = "tomato") +
#   geom_histogram(fill = "white", colour = "grey10", bins = 100) +
#   xlab("Caste difference in % methylation\n(Positive values = higher % methylation in workers)") +
#   ylab("Number of genes")
```
...
to_keep <- colnames(meth_matrix[, apply(meth_matrix, 2, function(x) sum(!is.na(x))) >= 10][,-1])
```


Here, we construct a sample-by-site matrix where each cell contains the proportion of methylated cytosines, containing all `r length(to_keep)` sites that were measured with 20x or better coverage in at least 10 samples. We then reduce the data to 16 components by non-metric multidimensional scaling (NMDS), preserving `r round(readRDS("output/meth_nmds_rs.rds")*100)`% of the variance in the original data. 
...
Before plotting, the mean % methylation of sites in gene $j$ has been mean-centered and scaled. Thus, orange indicates that the gene $j$ has a methylation frequency above the average frequency for all the samples analysed, and purple indicates a below-average frequencies.

```{r meth_sample_distance_heatmap}
meth_sample_heatmap <- function(){

  foc <- readRDS("data/Methylation_data/input_data_for_meth_network_CLEANED.rds") # created in code below
  foc <- foc[,apply(foc, 2, function(x) sum(!is.na(x))) >= 15]
  
  # Scale proportion methylation to have mean 0, variance 1
  for(i in 1:ncol(foc)) {
    foc[, i] <- as.numeric(scale(foc[, i]))
  }

  paletteLength <- 50
  myColor <- colorRampPalette(c(brewer.pal(9, "Purples")[7], "white", brewer.pal(9, "Oranges")[7]))(paletteLength)
  myBreaks <- c(seq(min(foc, na.rm=T), 0, length.out=ceiling(paletteLength/2) + 1), 
                seq(max(foc, na.rm=T)/paletteLength, 
                    max(foc, na.rm=T), length.out = floor(paletteLength/2)))
  ...
  annotation_row <- name_converter %>% select(new, caste) %>% as.data.frame()
...
The following code runs a Bayesian generalised additive model (GAM) on every site for which we have enough data; specifically, 'enough data' means:
...
# gene_meth_results_table %>% filter(sig_caste == "\\*" | sig_time_queen == "\\*" | sig_time_queen == "\\*") %>% pull(gene_name) %>% unique() %>% length()
...
bwasp_caste_sig_genes <- bwasp_caste_sig_sites %>% 
  ...
  mutate(`Also sig in Bayesian GAM analysis` = 
           ifelse(`Gene name` %in% intersection_caste, "Yes", "")) %>%
  arrange(desc(`Also sig in Bayesian GAM analysis`))
...
saveRDS(list(bwasp_caste_sig_genes=bwasp_caste_sig_genes, 
             bwasp_timeQ_sig_genes=bwasp_timeQ_sig_genes,
             bwasp_timeW_sig_genes=bwasp_timeW_sig_genes),
         file = "output/bwasp_sig_genes.rds")
```
...
bwasp_caste_sig_genes %>%
  ...
  scroll_box(height = "400px") 
```
**Table XX:** The table shows the names of the `r nrow(bwasp_caste_sig_genes)` genes in which BWASP detected one or more sites showing a significant difference in methylation between queens and workers at one or more time points. Other columns identify the number of significant sites found for each gene (total number: `r nrow(bwasp_caste_sig_sites)`), and state whether a significant result was also found for each gene in the Bayesian GAM analysis.
...
bwasp_timeQ_sig_genes %>%
  ...
  scroll_box(height = "400px") 
```
**Table XX:** The table shows the names of the `r nrow(bwasp_timeQ_sig_genes)` genes in which BWASP detected one or more sites showing a significant difference in methylation between one or more time points within the _queen_ samples. Other columns identify the number of significant sites found for each gene (total number: `r nrow(bwasp_timeQ_sig_sites)`), and state whether a significant result was also found for each gene in the Bayesian GAM analysis.
...
bwasp_timeW_sig_genes %>%
  ...
  scroll_box(height = "400px") 
```
**Table XX:** The table shows the names of the `r nrow(bwasp_timeW_sig_genes)` genes in which BWASP detected one or more sites showing a significant difference in methylation between one or more time points within the _worker_ samples. Other columns identify the number of significant sites found for each gene (total number: `r nrow(bwasp_timeW_sig_sites)`), and state whether a significant result was also found for each gene in the Bayesian GAM analysis.
...
bayesian_upset_data <- 
  map_df(model_results, ~ .x$caste_difference %>% 
      ...
      filter(p < 0.01) %>%
      left_join(annotations) %>% select(gene, comparison) %>%
...
bayesian_upset_data %>% 
  ...
  map(~ .x %>% pull(gene)) %>%
  fromList() %>%
  upset(order.by = "freq", nsets = 9999, nintersects = 17) 
```
...
if(!file.exists("output/upset_bwasp_data.rds")){
  ...
  upset_bwasp_data <- methyl_diff %>%
    filter(qvalue <= 0.01) %>% # get sig sites (using p<0.05 gets thousands more, possibly spurious, sites)
    select(gene_symbol, comparison) %>% # get unique gene names for these sig sites
    distinct() %>%
    split(.$comparison) %>%
    map(~ .x %>% pull(gene_symbol)) 
  
  saveRDS(upset_bwasp_data, "output/upset_bwasp_data.rds")
}

upset_bwasp_data <- readRDS("output/upset_bwasp_data.rds")
...
<!-- ```{r message=FALSE} -->
<!-- list( -->
<!--   bwasp_caste = upset_bwasp_data[ -->
<!--     str_detect(names(upset_bwasp_data), "queen") & str_detect(names(upset_bwasp_data), "worker")] %>% -->
<!--   unlist() %>% unname() %>% unique(), -->
<!--   bwasp_timeQ = upset_bwasp_data[str_detect(names(upset_bwasp_data), "queen") & !str_detect(names(upset_bwasp_data), "worker")] %>% -->
<!--   unlist() %>% unname() %>% unique(), -->
<!--   bwasp_timeW = upset_bwasp_data[!str_detect(names(upset_bwasp_data), "queen") & str_detect(names(upset_bwasp_data), "worker")] %>% -->
<!--   unlist() %>% unname() %>% unique(), -->
<!--   bayes_caste = annotations %>% filter(site %in% sig_sites_caste) %>% pull(gene) %>% unique(), -->
<!--   bayes_timeQ = annotations %>% filter(site %in% sig_sites_time_queen) %>% pull(gene) %>% unique(), -->
<!--   bayes_timeW = annotations %>% filter(site %in% sig_sites_time_worker) %>% pull(gene) %>% unique() -->
<!-- ) %>% -->
<!--   fromList() %>% -->
<!--   upset(order.by = "freq", nsets = 9999) -->
<!-- ``` -->
...
sites_in_bayesian_analysis <- map_df(model_results, ~ .x$caste_difference %>% 
      mutate(site = .x$site,
             comparison = glue("queen{time}.vs.worker{time}"))) %>%
      select(site, `Q-W_difference_percent_meth`, comparison) %>%
  bind_rows(map_df(model_results, ~ .x$time_difference %>% 
  ...
  select(site) %>%
  distinct() %>%
  mutate(seqnames = map_chr(strsplit(site, split = "~"), ~ .x[1]),
         start = map_chr(strsplit(site, split = "~"), ~ .x[2]),
         end = map_chr(strsplit(site, split = "~"), ~ .x[3]))



wide_bwasp <- vroom::vroom("data/methyl_diff_results/methyl_diff_results.tsv") %>%
  filter(seqnames %in% sites_in_bayesian_analysis$seqnames & start %in% sites_in_bayesian_analysis$start) %>%
...
There are 6 worker-biased modules (one of which is Module 0) and one module that did not show a caste bias (Module 3).
...
read_csv("output/eigengene_stats_table_meth.csv") %>% 
  ...
  mutate(Module = factor(Module, paste("Module", 0:6))) %>%
