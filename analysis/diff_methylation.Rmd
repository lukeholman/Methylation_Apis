...
We label a CpG site as significantly differentially methylated between castes/timepoints if the effect of caste (as measured by the best-fitting model) was $p$ < 0.0001 (n = `r length(sig_sites_caste_GLM)` sites). Not all sites survived false discovery rate correction; those that did are labelled with an asterisk the table below (we include both since the false discovery rate correction is overly conservative here, since these tests are not independent due to correlations in methylation between sites) (n = `r n_FDR_sig` sites were signficant after FDR correction). The table shows the location of each significant site, and mentioned if it is found in parts of one or more genes. 
