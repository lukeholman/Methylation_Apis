...
The following plots show estimates of the average difference in methylation between castes (within a timepoint) or timepoints (within a caste). These estimates were calculated by the BWASP pipelinee (which uses the methylkit package), and represent the unstandardised effect size, i.e. the difference in % methylated cytosines between teh two groups being compared. We then used a linear model to estimate the average effect size for sites of a particular type (e.g. sites in translated regions, or UTRs). This was accomplished by running a model with the effect size as the response variable, and the type of site as predictors (coded as binary variables, e.g. "in coding region" can be true or false). Note that all the site types except intergenic are not mutually exclusive, since a site can be e.g. in the promoter region of one gene and the exon of another; "intergenic" sites are those that do not map to the exons/introns/promoters of any known genes. 

### Comparing queen- and worker-destined larvae

```{r site_type_plots, message=FALSE, warning=FALSE, fig.width=4}
...
make_lm_estimates <- function(comp){
  ...
  if(str_detect(comp, "queen") & !str_detect(comp, "worker")) {
    facet_label <- paste("Queens:", paste0(str_extract_all(comp, "[02468]")[[1]], collapse = " vs "))
    plotting_data <- plotting_data %>% 
      filter(comparison == comp)
  }
  if(!str_detect(comp, "queen") & str_detect(comp, "worker")) {
    facet_label <- paste("Workers:", paste0(str_extract_all(comp, "[02468]")[[1]], collapse = " vs "))
    plotting_data <- plotting_data %>% 
      filter(comparison == comp)
  }
...
The following sections show the results of statistical tests using the BWASP pipeline. The table shows a list of sites or 500bp genomic windows that were statistically significantly differentially methylated(using Fisher Exact tests). Using BWASP, we specified a series of pre-planned comparions (contrasts) to compare the methylation levels at each site A) between castes witin each time point (e.g. queen vs worker samples at 8h), and B) between time points within each caste (e.g. 2h vs 8h within the queen samples). The tabs in each sex show sites/windows that were significantly differenetially methylated between castes (at a given time point), or between time points (within each caste).

The Fisher Exact test approach is under-powered because one must ignores all the other data when estimating each effect size. For example, to test for differential methylation in the 't0 vs 6h queen' comparison, one must ignore all the data from worker samples, or from the other time points in queens. The Fisher Exact test appraoch is also problematic because we have run to many separate models for each site (to calculate each of the pairwise comparisons), instead of running just a single model for each site as in the generalised additive models presented below. 

Nevertheless, the appraoch utilised in BWASP is the standard appraoch in the field, so it is helpful in that regard. Also, it is reassuring that some of our most interesting genes (e.g. _complementary sex determiner_) are significantly in the BWASP analysis as well as the Bayesian GAMs used below, despite the very different statistical approaches used. 
...
# the B methylation percentage is higher than the A methylation percentage.
...
between_castes %>% dplyr::select(-site) %>% my_data_table()
```
...
temporal_change_queens %>% dplyr::select(-site) %>% my_data_table()
```
...
### List of significantly differentially methylated **500bp windows** (DMWs) from BWASP {.tabset}
...
**Table XX:** The table shows the `r nrow(window_500bp_caste)` 500bp windows in which BWASP detected a significant difference in methylation between queens and workers at one or more time points. 
...
window_500bp_timeQ %>%
  my_data_table()
```
**Table XX:** The table shows the `r nrow(window_500bp_timeQ)` 500bp windows in which BWASP detected a significant change in methylation with time, in queen-destined larvae.
...
window_500bp_timeW %>%
  my_data_table()
```
**Table XX:** The table shows the `r nrow(window_500bp_timeW)` 500bp windows in which BWASP detected a significant change in methylation with time, in worker-destined larvae.
...
There are too many sites to run the Bayesian models on all of them in a reasonable amount of time. We therefore first use a model selection approach with frequentist generalised additive model (GAMs; implemented in the `mgcv` package, which runs a similar model in <1 second) to select a set of sites that appear to show a caste difference in the frequency of methylation, which we then re-analyse using the slower Bayesian approach.

- The analysis uses all the data except the time zero samples (which are complicated to model correctly because they do not neatly belong to either the queen or worker caste)
- `Time` is modelled using a thin plate regression spline, allowing non-linear changes with time, with the amount of non-linearity being driven by the data. 
- For each site, we fite 5 different models and then compare them using the corrected Akaike Information Criterion. The 5 models fitted included the following fixed effects: 1) neither caste nor time (a 'null model'), 2) caste, 3) time, 4) both caste and time, or 5) caste, time, and their interaction (the 'full model'). Additionally, all these models contained the experimental replicate as a fixed effect. 
- The formula of the full model was the same as for the Bayesian models run below, i.e. `caste + s(time, by = caste, k = 4) + rep`.
- The response variable was the number of cytosines observed at the site, given the total coverage at the sites (i.e. number of Cs plus number of Ts). We used Binomial errors and a logit link.
...
The previous code chunk identified `r length(sites_for_bayes_GAM)` for analysis, which we now analyse using a Bayesian generalised additive model (GAM). The Bayesian approach is more flexible, e.g. it allows us to incorporate the t0 samples into the analysis, facilitates calculation of 95% credible intervals for metrics of interest, and facilitates regularization using priors. 

Assuming that the focal dataset contained one or more t0 samples, we first generated multiple copies of the orignal dataset in which the t0 samples were relabelled as either a queen or worker. We generated all possible combinations of caste labels in which there were approximately even numbers of queens and workers; for example, if there were data on all four t0 samples for the focal site (the typical situation for most of the sites in our post-QC dataset), we relabelled the samples 6 different ways as follows: QQWW, QWQW, QWWQ, WQQW, WQWQ, WWQQ. We then ran a Bayesian GAM on each of these datasets with the relablled t0 samples, and then averaged their parameter estimates by combining their posteriors using the `brm_multiple` function in `brms`. This procedure will tend to push the estimate of % methylation at time zero towards the same value for both castes (which we know _a priori_ must be the case), by effectively making each t0 sample affect the parameter estimates for both queens and workers. 
...
The following runs the models in parallel using `future_lapply`, and we try to save time by not recompiling the model between each site. Still take more than a day to run. Be sure to run in R (not RStudio) in order to use forking for parallelisation.
...
In the figures, the orange colour indicates that a group of sites has a methylation frequency above the average for that site, while purple indicates a below-average methylation frequency. 

#### Heatmap of sites with $\ge 75$% methylation

```{r meth_sample_distance_heatmap, width = 11, height = 5.6, message=FALSE, warning=FALSE}
