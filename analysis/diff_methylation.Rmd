...
genomic_context_plot_caste <- 
  ...
  plot_lm_estimates("Difference in % mCpGs (mean \u00B195% CIs)")
...
```{r meth_sample_distance_heatmap, width = 10, height = 14, message=FALSE, warning=FALSE}
...
```{r message=FALSE, warning=FALSE, width = 10, height = 14}
set.seed(10)
...
saveRDS(heatmap_all, "figures/heatmap_all.rds")
heatmap_all
```
...
n_sig_sites <- readRDS("output/gene_meth_results_table.rds") %>% 
  ...
  select(site) %>% distinct() %>% nrow()

# to count the sig loci:
n_sig_sites <- gene_meth_results_table %>%
  filter(caste_pval <= p_thresh | timeQ_pval <= p_thresh | timeW_pval <= p_thresh) %>% pull(site) %>% unique() %>% length()
...
ggsave("figures/composite_figure.pdf", grid.arrange(
  arrangeGrob(readRDS("figures/histo_fig1.rds"), 
              readRDS("figures/genomic_context_plot_caste.rds") + 
                geom_point(fill = "#b19cd9", pch=21) + 
                xlab(NULL) + ylab("Mean caste difference in\n% mCpGs (\u00B195% CIs)"), 
