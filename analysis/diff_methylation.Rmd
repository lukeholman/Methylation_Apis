...
We label a site as significantly differentially methylated between castes/timepoints if...
...
gene_meth_results_table <- p_values %>%
  ...
  left_join(glm_results %>% group_by(Site) %>% 
              summarise(direction = ifelse(Caste_difference[which.max(abs(Caste_difference))] > 0, "Q>W", "W>Q")), by = c("site" = "Site")) %>%
  arrange(p) %>%
...
redo <- FALSE  # Make all the plots and save to disk for later display
...
![](https://raw.githubusercontent.com/lukeholman/Methylation_Apis/master/figures/bayes_sites/plot26.png)
