...
# FINISH THIS
# Save supplmenetary figures
remaining_terms <- fig_dat$term[!(fig_dat$term %in% top50)] %>% unique()
remaining_terms[1:(length(remaining_terms)/2)]


fig_dat %>%
  filter(term %in% remaining_terms[1:(length(remaining_terms)/2)]) %>%
  make_pathway_figure("", "Hours post-grafting") 

fig_dat %>%
  filter(term %in% remaining_terms[(1 + length(remaining_terms)/2):length(remaining_terms)]) %>%
  make_pathway_figure("", "Hours post-grafting") 
