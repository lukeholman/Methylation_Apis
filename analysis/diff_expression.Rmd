...
```{r filter_genes}
# Calculate normalisaiton factors for each library based on the the read count expression data
...
```{r nmds_transcriptome, fig.showtext=TRUE, fig.height=5, fig.width=7}
...
saveRDS(nmds_plot, "figures/expression_NMDS_plot.rds")
nmds_plot
```

**Figure**: Analysis of the transcriptome data using non-metric multidimensional scaling, which reduces the distance matrix between each sample's transcriptome to three dimensions (variance explained: `r round(100*nmds_r2, 1)`%). The plot suggests that queen- and worker-destined larvae have different gene expression profiles and that the transcriptome changes over time, and there is some indication that the caste difference is larger at later times post-grafting into a queen or worker cell. 
...
myBreaks <- c(seq(min(scaled_expression_data, na.rm=T), 0, 
                  length.out=ceiling(paletteLength/2) + 1), 
              seq(max(scaled_expression_data, na.rm=T)/paletteLength, 
                  max(scaled_expression_data, na.rm=T), length.out=floor(paletteLength/2)))
...
# Invisible code chunk, needed because pheatmap plots even if you don't want it to!
save_pheatmap_pdf <- function(x, filename, width=7, height=7) {
   stopifnot(!missing(x))
   stopifnot(!missing(filename))
   
   my_gtable <- x$gtable
   my_gtable$grobs[[4]]$gp <- gpar(fontfamily = nice_font) # x axis labels
   my_gtable$grobs[[7]]$gp <- gpar(fontfamily = nice_font) # Q/W legend
...
invisible(t(scaled_expression_data) %>%
  pheatmap(col = myColor, breaks = myBreaks,
           clustering_distance_rows = "euclidean",
           clustering_distance_cols = "euclidean",
           cutree_cols = 18, cutree_rows = 5, 
           annotation_row = annotation_row, annotation_colors = annotation_colors,
           show_colnames = FALSE) %>%
  save_pheatmap_pdf("figures/expression_heatmap.pdf", width = 12, height = 7))
```
...
### Counting the significant genes {.tabset}
...
In the following table, a **positive** log fold change in expression indicates **higher expression in QDL** relative to WDL measured at the same time point. One can also view this table on a full web page [here](caste_table.html) (including each gene's module membership according to the co-expression network analysis below), or press the buttons to export the table.

```{r intersections}
get_intersections <- function(results_df, split_by){
  results_df <- results_df %>%
    mutate(Gene = paste(`Gene symbol`, `Gene name`, sep = "~~")) %>%
    filter(adj.P.Val <= 0.05) # note: this is the criterion used for significant diff. expression
...
caste_table <- get_intersections(caste_results, "Time") %>% melt() %>% 
  mutate(
...
caste_table %>% write_csv("figures/supp_tables/tableS10.csv")
...
In the following table, a **positive** log fold change in expression indicates **higher expression at later times** than earlier times for a given type of larva (QDL or WDL). You can also view this table on a full web page [here](queen_time_table.html)  (including each gene's module membership according to the co-expression network analysis below), or press the buttons to export the table.

```{r temporal_Q}
timeQ_table <- time_results %>% 
  filter(Caste == "Queen") %>%
...
timeQ_table %>% write_csv("figures/supp_tables/tableS11.csv")
...
You can also view this table on a full web page [here](worker_time_table.html) (including each gene's module membership according to the co-expression network analysis below), or press the buttons to export the table to PDF, .csv, or Excel formats.

```{r temporal_W}
timeW_table <- time_results %>% 
  filter(Caste == "Worker") %>%
  get_intersections("Time") %>% 
  melt() %>% 
  mutate(
...
timeW_table %>% write_csv("figures/supp_tables/tableS12.csv")

timeW_table %>%
  my_data_table()

# %>%
#   datatable(rownames = FALSE, filter = 'top', fillContainer = TRUE,  # probably old code meant to delete?
...
## Enriched pathways among the differentially expressed genes 
...
### Enrichment results for genes showing __caste differences__ in expression
...
###### Figure
```{r fig.height=26, fig.width = 5.3}
fig_dat <- GO_KEGG_results$GO_Dmel_annotations_caste %>%
  filter(Test_type == "GO: Biological process") 

top50 <- fig_dat %>%
  group_by(term) %>%
  summarise(p = min(pval), .groups = "drop") %>% 
  arrange(p) %>% pull(term) %>% head(50)

fig_dat %>%
  filter(term %in% top50) %>%
  make_pathway_figure("", "Hours post-grafting") %>%
  saveRDS("figures/expr_GO_for_composite_figure.rds")
...
remaining_terms <- fig_dat$term[!(fig_dat$term %in% top50)] %>% unique()
remaining_terms_part1 <- remaining_terms[1:(length(remaining_terms)/2)]
remaining_terms_part2 <- remaining_terms[(1 + length(remaining_terms)/2):length(remaining_terms)]

# Biological process (not including the ones in the composite GO figure in the main paper)
left_side <- fig_dat %>%
  filter(term %in% remaining_terms_part1) %>%
  make_pathway_figure("", "Hours post-grafting")  +
  theme(text = element_text(family = nice_font),
...
right_side <- fig_dat %>%
  filter(term %in% remaining_terms_part2) %>%
  make_pathway_figure("", "Hours post-grafting")  +
  theme(text = element_text(family = nice_font),
        strip.text = element_blank(), 
        strip.background = element_blank(),
...
grid.arrange(left_side, right_side, ncol=2) %>%
  saveRDS("figures/expr_GO_BPremainder_dmel.rds")

# Molecular function
MF_terms <- GO_KEGG_results$GO_Dmel_annotations_caste %>%
  filter(Test_type == "GO: Molecular function") 
MF_terms_part1 <- MF_terms[1:(length(MF_terms)/2)]
MF_terms_part2 <- MF_terms[(1 + length(MF_terms)/2):length(MF_terms)]

left_side <- fig_dat %>%
  filter(term %in% MF_terms_part1) %>%
  make_pathway_figure("", "Hours post-grafting")  +
  theme(text = element_text(family = nice_font),
        strip.text = element_blank(), 
        strip.background = element_blank(), 
...
grid.arrange(left_side, right_side, ncol=2) %>%
  saveRDS("figures/expr_GO_MF_dmel.rds")
...
(GO_KEGG_results$GO_Dmel_annotations_caste %>%
  ...
  make_pathway_figure("", "Hours post-grafting") +
     theme(text = element_text(family = nice_font))) %>%
  saveRDS("figures/expr_GO_CC_dmel.rds")

(GO_KEGG_results$GO_Dmel_annotations_caste %>%
  filter(Test_type == "KEGG") %>%
  make_pathway_figure("", "Hours post-grafting") +
     theme(text = element_text(family = nice_font))) %>%
  saveRDS("figures/expr_KEGG_dmel.rds")
```
...
bind_rows(GO_KEGG_results$GO_Dmel_annotations_temporalQ,
  ...
  make_pathway_figure("", "Hours post-grafting",  alt_colours = TRUE) %>%
  saveRDS("figures/expr_temporal_GOBP_dmel.rds")
...
bind_rows(GO_KEGG_results$GO_Dmel_annotations_temporalQ,
  ...
  make_pathway_figure("", "Hours post-grafting",  alt_colours = TRUE) %>%
  saveRDS("figures/expr_temporal_GOMF_dmel.rds")
...
bind_rows(GO_KEGG_results$GO_Dmel_annotations_temporalQ,
  ...
  filter(Test_type == "GO: Cellular component") %>%
  make_pathway_figure("", "Hours post-grafting", alt_colours = TRUE) %>%
  saveRDS("figures/expr_temporal_GOCC_dmel.rds")
...
bind_rows(GO_KEGG_results$GO_Dmel_annotations_temporalQ,
  ...
  filter(Test_type == "KEGG") %>%
  make_pathway_figure("", "Hours post-grafting", alt_colours = TRUE) %>%
  saveRDS("figures/expr_temporal_KEGG_dmel.rds")
...
if(!file.exists("output/expression_eigengenes.csv")){
  ...
  write_csv(caste_results, path = "output/caste_results_with_modules.csv")
  write_csv(time_results, path = "output/time_results_with_modules.csv")
...
rownames(mat) <- str_replace_all(rownames(mat), "2", "2h rep")
rownames(mat) <- str_replace_all(rownames(mat), "4", "4h rep")
rownames(mat) <- str_replace_all(rownames(mat), "6", "6h rep")
rownames(mat) <- str_replace_all(rownames(mat), "8", "8h rep")
...
where $y$ is the eigengene for module $j$. We specified regularising priors on all fixed effects estimates (i.e. `Normal(0, 2)`) to minimise overfitting to the data. 
...
brms_formula <- bf(
  paste("mvbind(",
        paste0(paste("m", 0:(j - 1), 
                     sep = ""), collapse = ","),
        ") ~ Caste:Time + I(Time^2) + I(Time^2):Caste + Replicate", sep = "") 
) + set_rescor(FALSE)   
...
raw_data <- brms_data[[1]] %>%
  ...
  mutate(Module = str_replace_all(Module, "m", "Module "),
    ...
    mutate(Caste = replace(as.character(Caste), Time == 0, "Pre-grafting"),
           Caste = replace(Caste, Caste == "q", "Queen"),
...
means_data <- preds %>%
  mutate(Caste = replace(Caste, is.na(Caste), "Pre-grafting")) %>%
...
mean_line_data <- preds %>%
  mutate(Caste = replace(Caste, is.na(Caste), "Pre-grafting")) %>%
  group_by(Module, Caste, Time) %>%
  summarise(stats = list(as.data.frame(posterior_summary(eigengene))), .groups = "drop") %>%
  ungroup() %>% unnest(stats) %>% dplyr::rename(eigengene = Estimate)
...
mean_line_data <- left_join(mean_line_data, gene_number_labels, by = "Module") %>%
  mutate(Caste = replace(Caste, Caste == "Pre-grafting", NA))

# Extend the red and blue lines to zero
preds <- bind_rows(
  preds[is.na(preds$Caste), ] %>% mutate(Caste = "Queen"),
  preds[is.na(preds$Caste), ] %>% mutate(Caste = "Worker"),
  preds[!is.na(preds$Caste), ])

mean_line_data <- bind_rows(
  mean_line_data[is.na(mean_line_data$Caste), ] %>% mutate(Caste = "Queen"),
  mean_line_data[is.na(mean_line_data$Caste), ] %>% mutate(Caste = "Worker"),
  mean_line_data[!is.na(mean_line_data$Caste), ])
...
eigen_plot <- preds %>%
  ...
  geom_point(data = means_data, aes(fill = Caste),
             pch = 21, size = 1, alpha = 0.7, position = pd) +
...
manual_GO_enrich <- function(
  ...
  # odds ratio: odds that the focal GO is in the set / odds it's in the universe
...
present_module_GO_table <- function(module_GO_tbl){
  ...
  focal %>%
    dplyr::select(Module, everything()) %>%
    mutate(Module = str_remove_all(Module, " ")) %>%
    mutate_if(is.numeric, ~ round(.x, 3)) 
}

present_module_GO_table(module_GO_table_dmel) %>%
  write_csv("output/module_GO_table_expr.csv")
