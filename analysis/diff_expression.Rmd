...
```{r}
set.seed(1)
nmds_output <- dist(expression_data) %>% nmds(maxdim = 16)
saveRDS(tail(nmds_output$r2, 1), "output/expression_data_nmds_rs.rds")
...
```{r nmds1}
nmds_output <- dist(t(expression_data)) %>% nmds(maxdim = 1)
...
nmds_output <- data.frame(Caste = sample_ids$caste,
                          ...
                          NMDS_score= nmds_output$conf[[length(nmds_output$conf)]]) %>%
  mutate(NMDS_score = as.numeric(scale(NMDS_score)))
 ...
 bind_rows(nmds_output %>% filter(Time == 0) %>% mutate(Caste = "Queen"),
  ...
  ggplot(aes(Time, NMDS_score, colour = Caste, group = Caste)) + 
  geom_line() +
  geom_point() + 
  scale_colour_manual(values = c(queen_colour, worker_colour), 
                      name = "Cell type") + 
...
Here, the expression level of gene $j$ in transcriptome $i$ has been converted to a proportion of the total, then mean-centered and scaled. Thus, red indicates that the gene $j$ makes up an above-average fraction of transcriptome $i$, and blue indicates a below-average fraction.

Samples don't cluster perfectly (e.g. there is one 2h Worker sample clustering with some 6h and 8h Queen samples), perhaps because of batch effects (note the clustering by replicate). However, there is evident differentiation by caste and time for parts of the transcriptome. The 'transcriptional modules' that we later analyse using WGCNA are also apparent. 

```{r bigheatmap, fig.height=9}
