...
#expression_data_old <- as.matrix(read.table("data/Expression_data/Gene.txt", row.names = 1, header = T))
expression_data <- as.matrix(read.table("data/Expression_data/Genes_2020_version.txt", row.names = 1, header = T))
time0_samples <- colnames(expression_data)[str_detect(colnames(expression_data), "t0")]
...
sample_ids <- data.frame(
  ...
  mutate(caste = replace(as.character(caste), str_detect(colnames(expression_data), "t0"), "Time zero"),
         time_point = relevel(time_point, ref = "2"),
...
tibble(` ` = c("Number of transcripts prior to filtering:", 
               "Number remaining after filtering out transcripts where max(CPM) < 1:"),
...
#### Bivariate plot by caste and time point
```{r}
nmds_output %>%
  mutate_at(vars(starts_with("X")), ~ as.numeric(scale(.x))) %>%
  gather(NMDS, Score, starts_with("X")) %>%
...
```{r enrich1, warning = FALSE, fig.height=10.1, fig.width = 5.3}
...
make_pathway_figure <- function(df, ylab){
  df %>%
    ...
    geom_tile(colour = "black", stroke = 0.4)  + 
...
```{r enrich2, fig.height=10, fig.width = 5.3}
MF_fig
```

### GO: Cellular component
```{r enrich3, fig.width = 5.3}
CC_fig
```

### KEGG
```{r enrich4, fig.height=14, fig.width = 5.3}
kegg_fig
```
...
build_network <- function(expression_data){
  ...
  for(i in ncol(my_data)) my_data[,i] <- as.numeric(my_data[,i])
  ...
  soft_power <- pickSoftThreshold(
    my_data, RsquaredCut = 0.85, verbose = 0, powerVector = 1:30)
...
brms_data <- eigengenes %>%
  ...
  mutate(Caste = C(Caste, contr = "contr.sum")) 
...
$$ y \sim Caste + s(Time, by = Caste, k = 4) + (1 | Replicate)$$
...
brms_formula <- bf(
  paste("mvbind(",
        paste0(paste("m", 0:(j - 1), 
                     sep = ""), collapse = ","),
        ") ~ Caste + s(Time, by = Caste, k = 4) + (1 | Replicate)", sep = "")
) + set_rescor(FALSE)   
...
run_brms_eigengene_model <- function() {
  brm(
    brms_formula,
    data = brms_data,
    prior = c(set_prior("normal(0, 6)", class = "b")), 
    chains = 1, iter = 4000, cores = 1, inits = 0,
    control = list(adapt_delta = 0.9999, max_treedepth = 20))
}

if(!file.exists("data/brms_eigengene_model.rds")){
  ...
  saveRDS(brms_eigengene_model, "data/brms_eigengene_model.rds")
} else brms_eigengene_model <- readRDS("data/brms_eigengene_model.rds")
...
#   model_fit <- glm(y ~ Caste * Time + Replicate, data = eigengene_model_data)
#   
...
# }) %>% bind_rows() %>% 
#   rename(interaction = `Caste:Time`) %>%
#   mutate(Formula = "Intercept only", 
#          Formula = replace(Formula, !is.na(Caste), "Caste"), 
#          Formula = replace(Formula, !is.na(Time), "Time"),
#          Formula = replace(Formula, !is.na(Caste) & !is.na(Time), "Caste + Time"),
#          Formula = replace(Formula, !is.na(interaction), "Caste x Time")) %>%
...
make_preds <- function(){
  preds <- fitted(brms_eigengene_model, new, summary = FALSE, re_formula = NA)
...
gene_number_labels <- dplyr::rename(network$colors %>% table() %>% 
  ...
  mutate(label_text = paste(n, "genes"),
         Time = 8.21, eigengene = 3.2,
         Module = factor(Module, mod_levels))

# Make the figure
pd <- position_dodge(0.35)
col1 <- "#d13b40"
col2 <- RColorBrewer::brewer.pal(7, "Blues")[4]

eigen_plot <- preds %>%
  filter(draw %in% 1:100) %>% # plot 100 posterior fit lines
  ...
  geom_text(data = gene_number_labels, aes(label = label_text), 
            size = 2.8, hjust = 1, colour = "#565d47", family = nice_font) + 
  ...
  geom_point(data = raw_data, alpha = 0.7, aes(fill = Caste),
             size = 0.9, pch = 21, colour = "black", stroke = 0.3, position = pd) + 
  # Add the posterior median +/- error bars
  geom_errorbar(data = means_data, 
                width = 0, alpha = 0.7,
                aes(ymin = eigengene-Est.Error, colour = Caste,
                    ymax = eigengene+Est.Error), position = pd) +
  geom_point(data = means_data, aes(fill = Caste), pch = 21, size = 2, alpha = 0.7, position = pd) +
  ...
  coord_cartesian(ylim = ylimits, xlim = c(2,8)) + 
  theme_bw() + 
  theme(legend.position = "top",
        text = element_text(family = nice_font),
        strip.background = element_rect(fill = "white", colour = "white"))

ggsave(eigen_plot, filename = "figures/eigengene_figure.pdf", width = 8, height = 8)
eigen_plot
```
...
<!--   mutate(Module = paste(gsub("Module ", "M", Module), ": ", -->
...
<!--          Caste = replace(Caste, Caste == "Time zero", "Pre-manipulation"), -->
<!--          Caste = replace(Caste, Caste == "Queen", "Queen cell"), -->
<!--          Caste = replace(Caste, Caste == "Worker", "Worker cell")) %>% -->
<!--   ggplot(aes(Time, Eigengene, fill = Caste)) +  -->
<!--   geom_boxplot() +  -->
<!--   scale_fill_manual(values = c("purple", "#fc766a", "skyblue"), name = NULL) + # nice warm yellow #fed65e -->
...
# 
# 
# GO.and.KEGG.hypergeometric <- function(
#   gene_set, gene_universe, apis.db, min.size = 5, keep.all = FALSE){
#   
...
#     is_enriched <- sapply(
#       result$GeneRatio, 
#       function(x) eval(parse(text=x))) > sapply(result$BgRatio, function(x) eval(parse(text=x)))
...
#     
#     result <-  enrichKEGG(
#       gene_set, organism = "ame", keyType = "kegg", pvalueCutoff = p,
...
#   filter <- dplyr::filter
#   apis.db <- hub[["AH76825"]]
#   gene_universe_modules <- converter$entrez_id[
#     converter$gene_symbol %in% rownames(expression_data)] %>% as.character() 
#   
...
GO_and_KEGG_hypergeometric <- function(
  ...
  GO_enrichment <- function(gene_set,
    ...
    gene_set <- tbl(db, "bee_GO") %>%
      left_join(tbl(db, "gene_names"), by = c("SYMBOL" = "gene")) %>%
...
make_module_GO_table <- function(eigengenes, min_size = 5, keep_all = FALSE){
...
Note that no GO terms were significantly enriched for Module 0. Click the tabs to see the results for Modules 1-`r num_modules`.

#### Module 0

```{r}
make_module_GO_table <- function(module){
  
  focal <- module_GO_table %>%
  ...
  focal %>%
    ...
    datatable(options = list(pageLength = 5), #fillContainer = TRUE,
              rownames = FALSE)
}
make_module_GO_table(0) 
```

#### Module 1
```{r}
make_module_GO_table(1) 
```

#### Module 2
```{r}
make_module_GO_table(2) 
```

#### Module 3
```{r}
make_module_GO_table(3) 
```

#### Module 4
```{r}
make_module_GO_table(4) 
```

#### Module 5
```{r}
make_module_GO_table(5) 
```

#### Module 6
```{r}
make_module_GO_table(6) 
# similar to m12, the other caste-like one
```

#### Module 7
```{r}
make_module_GO_table(7) 
```

#### Module 8
```{r}
make_module_GO_table(8) # ox-phos in mitos. Caste specificity flips across development
```

#### Module 9
```{r} 
make_module_GO_table(9)
```

#### Module 10
```{r}
make_module_GO_table(10)
```

#### Module 11
```{r}
make_module_GO_table(11)
```

#### Module 12
```{r}
make_module_GO_table(12)
# conversion of Glycine, serine and threonine into pyruvate in the mitochondria, to make glucose?
```

#### Module 13
```{r}
make_module_GO_table(13)
```

#### Module 14
```{r}
make_module_GO_table(14)
```

#### Module 15
```{r}
make_module_GO_table(15)
# another caste one all about proteins and fats
```

#### Module 16
```{r}
make_module_GO_table(16)
```

#### Module 17
```{r}
make_module_GO_table(17)
```

#### Module 18
```{r}
make_module_GO_table(18)
```
