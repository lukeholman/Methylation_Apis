...
#### Univariate plot by caste and time point
```{r nmds1}
...
nmds_output <- data.frame(Caste = sample_ids$caste,
                          Time = as.numeric(sample_ids$time_point),
...
You can also view this table on a full web page [here](caste_table.html).
...
You can also view this table on a full web page [here](queen_time_table.html).
...
You can also view this table on a full web page [here](worker_time_table.html).
...
time_results %>% 
  ...
  datatable(options = list(pageLength = 10), 
            rownames = FALSE, filter = 'top', fillContainer = TRUE)
```
...
# df needs to have columns called `Gene symbol` and logFC
...
GO_and_KEGG_gsea <- function(df, GO_list, min.size = 5, keep.all = FALSE){
  ...
  # a named, ranked vector of logFC per gene
  geneList <- df$logFC
  ...
  kegg.enrichment <- function(geneList, GO_list){
    ...
    result <- fgsea::fgsea(pathways, geneList, minSize = min.size, maxSize = 500) %>% 
      filter(pval <= p) 
    collapse_pathways <- fgsea::collapsePathways(result, pathways, geneList, nperm = 1000)
...
all_GO_Amel_annotations <- rbind(
  caste_results %>% filter(Time == 2) %>% 
    GO_and_KEGG_gsea(keep.all = TRUE, GO_list = "bee_GO") %>% mutate(Time = 2),
  caste_results %>% filter(Time == 4) %>% 
    GO_and_KEGG_gsea(keep.all = TRUE, GO_list = "bee_GO") %>% mutate(Time = 4),
  caste_results %>% filter(Time == 6) %>% 
    GO_and_KEGG_gsea(keep.all = TRUE, GO_list = "bee_GO") %>% mutate(Time = 6),
  caste_results %>% filter(Time == 8) %>% 
    GO_and_KEGG_gsea(keep.all = TRUE, GO_list = "bee_GO") %>% mutate(Time = 8)
) %>% as_tibble()


all_GO_Dmel_annotations <- rbind(
  caste_results %>% filter(Time == 2) %>% 
    GO_and_KEGG_gsea(keep.all = TRUE, GO_list = "dros_ortho_GO") %>% mutate(Time = 2),
  caste_results %>% filter(Time == 4) %>% 
    GO_and_KEGG_gsea(keep.all = TRUE, GO_list = "dros_ortho_GO") %>% mutate(Time = 4),
  caste_results %>% filter(Time == 6) %>% 
    GO_and_KEGG_gsea(keep.all = TRUE, GO_list = "dros_ortho_GO") %>% mutate(Time = 6),
  caste_results %>% filter(Time == 8) %>% 
    GO_and_KEGG_gsea(keep.all = TRUE, GO_list = "dros_ortho_GO") %>% mutate(Time = 8)
) %>% as_tibble()

# Filter to just those pathways significant at one or more time points
sig_Amel <- all_GO_Amel_annotations %>%
  group_by(pathway) %>%
  summarise(min_p = min(padj)) %>%
  filter(min_p < 0.05) %>%
  pull(pathway)

sig_Dmel <- all_GO_Dmel_annotations %>%
  group_by(pathway) %>%
  summarise(min_p = min(padj)) %>%
  filter(min_p < 0.05) %>%
  pull(pathway)
...
all_GO_Amel_annotations <- all_GO_Amel_annotations %>% 
  filter(pathway %in% sig_Amel) 

all_GO_Dmel_annotations <- all_GO_Dmel_annotations %>% 
  filter(pathway %in% sig_Dmel) 

# Determine order for the figures and code up the asterisks
levels_Amel <- all_GO_Amel_annotations %>%
  mutate(term_wrapped = str_wrap(term, width = 50)) %>%
  group_by(term_wrapped) %>%
  summarise(p = prod(padj)^0.25) %>%
  arrange(p) %>% pull(term_wrapped) %>% rev()

levels_Dmel <- all_GO_Dmel_annotations %>%
  mutate(term_wrapped = str_wrap(term, width = 50)) %>%
  group_by(term_wrapped) %>%
  summarise(p = prod(padj)^0.25) %>%
  arrange(p) %>% pull(term_wrapped) %>% rev()
...
all_GO_Amel_annotations <- all_GO_Amel_annotations %>%
  ...
  mutate(term = factor(term, levels_Amel),
         sig = ifelse(pval < 0.05, "*", ""),
         sig = replace(sig, padj < 0.05, "**"))

all_GO_Dmel_annotations <- all_GO_Dmel_annotations %>%
  mutate(term = str_wrap(term, width = 50)) %>%
  mutate(term = factor(term, levels_Dmel),
         sig = ifelse(pval < 0.05, "*", ""),
         sig = replace(sig, padj < 0.05, "**"))
...
make_pathway_figure <- function(df, ylab){
  df %>%
    mutate(Time = as.character(Time)) %>%
    ggplot(aes(Time, term, fill = NES)) + 
    scale_fill_gradient2(low = brewer.pal(9, "Blues")[7], high = brewer.pal(9, "Reds")[7]) + 
    geom_tile(colour = "black", size = 0.4)  + 
    geom_text(aes(label = sig)) +
    theme_bw() + ylab(ylab) + xlab("Hours post-grafting") +
    scale_x_discrete(expand = c(0,0)) + 
    scale_y_discrete(expand = c(0,0)) 
}

BP_fig_Amel <- all_GO_Amel_annotations %>%
  filter(Test_type == "GO: Biological process") %>%
  make_pathway_figure("GO: Biological process")

MF_fig_Amel <- all_GO_Amel_annotations %>%
  filter(Test_type == "GO: Molecular function") %>%
  make_pathway_figure("GO: Molecular function")

CC_fig_Amel <- all_GO_Amel_annotations %>%
  filter(Test_type == "GO: Cellular component") %>%
  make_pathway_figure("GO: Cellular component")

kegg_fig_Amel <- all_GO_Amel_annotations %>%
  filter(Test_type == "KEGG") %>%
  make_pathway_figure("KEGG")

BP_fig_Dmel <- all_GO_Dmel_annotations %>%
  filter(Test_type == "GO: Biological process") %>%
  make_pathway_figure("GO: Biological process")

MF_fig_Dmel <- all_GO_Dmel_annotations %>%
  filter(Test_type == "GO: Molecular function") %>%
  make_pathway_figure("GO: Molecular function")

CC_fig_Dmel <- all_GO_Dmel_annotations %>%
  filter(Test_type == "GO: Cellular component") %>%
  make_pathway_figure("GO: Cellular component")

kegg_fig_Dmel <- all_GO_Dmel_annotations %>%
  filter(Test_type == "KEGG") %>%
  make_pathway_figure("KEGG")
```
...
#### GO: Biological process
```{r enrichBPmel, fig.height=10, fig.width = 5.3}
BP_fig_Dmel
```

####GO: Molecular function
```{r enrichMFmel, fig.height=10, fig.width = 5.3}
MF_fig_Dmel
```

#### GO: Cellular component
```{r enrichCCmel, fig.height=14, fig.width = 5.3}
CC_fig_Dmel
```
...
if(!file.exists("output/expression_eigengenes.csv")){
  ...
  write_csv(eigengenes, path = "output/expression_eigengenes.csv")
...
#     # get GO annotations for genes in the focal set/module
#     gene_set <- tbl(db, GO_list) %>%
#       left_join(tbl(db, "gene_names"), by = c("SYMBOL" = "gene_symbol")) %>%
#       dplyr::rename(gene = SYMBOL) %>%
#       dplyr::select(gene, GO, gene_name) %>%
#       filter(gene %in% gene_set) %>%
#       collect(n=Inf) %>%
...
#     }) 
#     
#     if(!keep_all) output <- output %>% filter(pvalue < 0.05)
#     if(nrow(output)==0) return(NULL)
#     
#     output %>%
#       left_join(tbl(db, "go_meanings") %>% collect(),
...
#   
#   # Define internal KEGG enrichment function (uses clusterProfiler)
#   kegg_enrichment <- function(gene_set_entrez, 
#                               gene_universe_entrez, min_size, keep_all){
#     
#     result <-  enrichKEGG(
#       gene_set_entrez, organism = "ame", keyType = "kegg", pvalueCutoff = p,
#       gene_universe_entrez, minGSSize = min_size, maxGSSize = 500,
#       qvalueCutoff = 1, use_internal_data = FALSE, pAdjustMethod = "BH")
#     
#     if(is.null(result)) return(NULL)
#     
...
#   }
#   
#   kegg_result <- kegg_enrichment(gene_set_entrez, gene_universe_entrez, min_size, keep_all) 
#   
#   if(!is.null(kegg_result)){
#     kegg_result <- kegg_result %>%
#       dplyr::rename(Meaning = Description) %>%
...
#   gene_universe_entrez <- converter$entrez_id[
#     converter$gene_symbol %in% gene_universe] %>% 
#     as.character() # just to be sure Entrez numbers are treated as strings
#   
#   map_df(1:n_modules, function(i) { 
#     
#     print(paste("Doing", i, "/", n_modules, "modules"))
#     
#     # Entrez IDs of the genes in the focal module
#     gene_set <- genes_in_module(i)
#     gene_set_entrez <- converter$entrez_id[
#       converter$gene_symbol %in% gene_set] %>% as.character()
#     
#     GO_and_KEGG_hypergeometric(
#       gene_set = gene_set, 
#       gene_universe = gene_universe, 
...
#       mutate(Module = paste("Module", i)) %>%
#       dplyr::select(Module, everything())
#   }) %>% mutate( # manually fix couple of missing ones
#     Ontology = replace(Ontology, ID == "GO:0001104", "MF"),
#     Meaning = replace(Meaning, ID == "GO:0001104", "transcription coregulator activity"),
#     Ontology = replace(Ontology, ID == "GO:0005578", "CC"),
#     Meaning = replace(Meaning, ID == "GO:0005578", "extracellular matrix"),
#     Ontology = replace(Ontology, ID == "GO:0022891", "MF"),
#     Meaning = replace(Meaning, ID == "GO:0022891", "transmembrane transporter activity")) %>%
#     distinct() %>%
#     mutate(Ontology = str_replace_all(Ontology, "BP", "GO:BP"),
#            Ontology = str_replace_all(Ontology, "MF", "GO:MF"),
#            Ontology = str_replace_all(Ontology, "CC", "GO:CC"),
#            Ontology = factor(Ontology, c("GO:BP", "GO:MF", "GO:CC", "KEGG")))
# }
...
### Tables of the module enrichment results (_A. mellifera_ annotations) {.tabset}

Note that no GO terms were significantly enriched for Module 0. Click the tabs to see the results for Modules 1-`r num_modules-1`.

#### Module 0

```{r}
present_module_GO_table <- function(module, module_GO_tbl){
...
present_module_GO_table(0, module_GO_table_amel) 
```

#### Module 1
```{r}
present_module_GO_table(1, module_GO_table_amel) 
```

#### Module 2
```{r}
present_module_GO_table(2, module_GO_table_amel) 
```

#### Module 3
```{r}
present_module_GO_table(3, module_GO_table_amel) 
```

#### Module 4
```{r}
present_module_GO_table(4, module_GO_table_amel) 
```

#### Module 5
```{r}
present_module_GO_table(5, module_GO_table_amel) 
```

#### Module 6
```{r}
present_module_GO_table(6, module_GO_table_amel) 
```

#### Module 7
```{r}
present_module_GO_table(7, module_GO_table_amel) 
```

#### Module 8
```{r}
present_module_GO_table(8, module_GO_table_amel)
```

#### Module 9
```{r} 
present_module_GO_table(9, module_GO_table_amel)
```

#### Module 10
```{r}
present_module_GO_table(10, module_GO_table_amel)
```

#### Module 11
```{r}
present_module_GO_table(11, module_GO_table_amel)
```

#### Module 12
```{r}
present_module_GO_table(12, module_GO_table_amel)
```

#### Module 13
```{r}
present_module_GO_table(13, module_GO_table_amel)
```

#### Module 14
```{r}
present_module_GO_table(14, module_GO_table_amel)
```

#### Module 15
```{r}
present_module_GO_table(15, module_GO_table_amel)
```

#### Module 16
```{r}
present_module_GO_table(16, module_GO_table_amel)
```

#### Module 17
```{r}
present_module_GO_table(17, module_GO_table_amel)
```

#### Module 18
```{r}
present_module_GO_table(18, module_GO_table_amel)
```


### Tables of the module enrichment results (_D. melanogaster_ annotations) {.tabset}
...
```{r}
present_module_GO_table(0, module_GO_table_dmel) 
```

#### Module 1
```{r}
present_module_GO_table(1, module_GO_table_dmel) 
```

#### Module 2
```{r}
present_module_GO_table(2, module_GO_table_dmel) 
```

#### Module 3
```{r}
present_module_GO_table(3, module_GO_table_dmel) 
```

#### Module 4
```{r}
present_module_GO_table(4, module_GO_table_dmel) 
```

#### Module 5
```{r}
present_module_GO_table(5, module_GO_table_dmel) 
```

#### Module 6
```{r}
present_module_GO_table(6, module_GO_table_dmel) 
```

#### Module 7
```{r}
present_module_GO_table(7, module_GO_table_dmel) 
```

#### Module 8
```{r}
present_module_GO_table(8, module_GO_table_dmel)
```

#### Module 9
```{r} 
present_module_GO_table(9, module_GO_table_dmel)
```

#### Module 10
```{r}
present_module_GO_table(10, module_GO_table_dmel)
```

#### Module 11
```{r}
present_module_GO_table(11, module_GO_table_dmel)
```

#### Module 12
```{r}
present_module_GO_table(12, module_GO_table_dmel)
```

#### Module 13
```{r}
present_module_GO_table(13, module_GO_table_dmel)
```

#### Module 14
```{r}
present_module_GO_table(14, module_GO_table_dmel)
```

#### Module 15
```{r}
present_module_GO_table(15, module_GO_table_dmel)
```

#### Module 16
```{r}
present_module_GO_table(16, module_GO_table_dmel)
```

#### Module 17
```{r}
present_module_GO_table(17, module_GO_table_dmel)
```

#### Module 18
```{r}
present_module_GO_table(18, module_GO_table_dmel)
```
...
make_a_plot <- function(yvar){
  ...
  merged_data <- merged_data %>% 
    dplyr::select(`Upregulation in queen-destined 8h larvae`, y)
  ...
  p <- ggplot(merged_data, aes(`Upregulation in queen-destined 8h larvae`, y)) + 
...
plots <- var[var != "Upregulation in queen-destined 8h larvae"] %>%
...
## Test idea about the doublesex module
```{r eval=FALSE}
...
# Find overlaps with our modules, and the dsx-containing module in velasque et al
module_overlaps <- caste_results %>%
...
caste_results %>%
  filter(adj.P.Val <= 0.05) %>%
  right_join(read_csv("data/apis_gene_comparisons/velasque_doublesex.csv"), by = c(`Gene symbol` = "gene")) %>%
