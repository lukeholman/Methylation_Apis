...
## List of genes showing significant temporal variation in queen-destined larvae
...
## List of genes showing significant temporal variation in worker-destined larvae
...
Red means relatively higher expression of the focal pathway/process in queens, blue means higher in workers. Calculated separately for each of the 4 time points.
...
GO_and_KEGG_gsea <- function(df, min.size = 5, keep.all = FALSE){
  ...
  GO.enrichment <- function(geneList, ontol){
    ...
    pathways <- tbl(db, "bee_GO") %>% 
      left_join(tbl(db, "go_meanings"), by = c("GO")) %>% # NEW LINE ADDED 12 MAY 2020
      distinct() %>%
      collect(n=Inf) %>%
      filter(SYMBOL %in% gene_universe, 
             ontology == ontol) %>%        # used to be ONTOLOGY
      select(-ontology) 
...
```{r echo=FALSE}
num_modules <- length(unique(network$colours))
```
...
module_GO_table <- make_module_GO_table(eigengenes) %>%
  arrange(Module, Test_type, -qvalue) %>%
  dplyr::select(-Count)
```
...
make_module_GO_table <- function(module){
  ...
  focal <- module_GO_table %>%
    dplyr::select(-sig, -qvalue) %>%
    arrange(Module, Test_type, pvalue) %>%
    filter(Module == paste("Module", module) & pvalue < 0.05)  %>%
    mutate(`Gene & Background ratios` = paste(GeneRatio, BgRatio),
           ID = paste(Description, " (", ID, ")", sep = "")) %>%
    dplyr::rename(Enrichment = enrichment) %>%
    dplyr::select(-GeneRatio, -BgRatio, -Description) %>%
    dplyr::select(Test_type, ID, `Gene & Background ratios`, everything())
  
  bp_rows <- which(focal$Test_type == "GO: Biological process")
  mf_rows <- which(focal$Test_type == "GO: Molecular function")
  cc_rows <- which(focal$Test_type == "GO: Cellular component")
  kg_rows <- which(focal$Test_type == "KEGG")
  ...
  #   kable(digits = 3) %>%
  #   kable_styling() %>%
  #   row_spec(bp_rows, bold = T, color = "black", background = scales::alpha("red", 0.1)) %>%
  #   row_spec(mf_rows, bold = T, color = "black", background = scales::alpha("blue", 0.1)) %>%
  #   row_spec(cc_rows, bold = T, color = "black", background = scales::alpha("green", 0.1)) %>%
  #   row_spec(kg_rows, bold = T, color = "black", background = scales::alpha("purple", 0.1)) 
}
...
#   mutate_all(~ as.numeric(scale((.x)))) %>%
# #  dplyr::select(`Upregulation in queen-destined 8h larvae`, starts_with("Warner")) %>%
