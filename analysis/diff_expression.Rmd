...
Here, the expression level of gene $j$ in transcriptome $i$ has been converted to a proportion of the total, then mean-centered and scaled. Thus, orange indicates that the gene $j$ makes up an above-average fraction of transcriptome $i$, and purple indicates a below-average fraction.

Samples don't cluster perfectly, though this may be partly because of batch effects (note the clustering by replicate). However, there is evident differentiation by caste and time for parts of the transcriptome. The 'transcriptional modules' that we later analyse using WGCNA are also apparent. 
